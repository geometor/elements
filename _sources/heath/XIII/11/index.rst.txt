:order: 11
:number: 600
:type: prop
:tags: line, circle
:dependencies: I.32, V.18, V.19.p.1, X.12, X.15, X.73, X.9, X.94, X.def.3.4, XIII.1, XIII.8




.. figure:: XIII.11.graphic.inverted.png

.. _XIII.11:

XIII.11
=======

   If in a circle which has its diameter rational an equilateral pentagon be inscribed, the side of the pentagon is the irrational straight line called minor.

For in the circle ABCDE which has its diameter rational let the equilateral pentagon ABCDE be inscribed; I say that the side of the pentagon is the irrational straight line called minor.

For let the centre of the circle, the point F, be taken, let AF, FB be joined and carried through to the points, G, H, let AC be joined, and let FK be made a fourth part of AF.

Now AF is rational; therefore FK is also rational.

But BF is also rational; therefore the whole BK is rational.

And, since the circumference ACG is equal to the circumference ADG, and in them ABC is equal to AED, therefore the remainder CG is equal to the remainder GD.

And, if we join AD, we conclude that the angles at L are right, and CD is double of CL.

For the same reason the angles at M are also right, and AC is double of CM.

Since then the angle ALC is equal to the angle AMF, and the angle LAC is common to the two triangles ACL and AMF, therefore the remaining angle ACL is equal to the remaining angle MFA; [:ref:`I.32`] therefore the triangle ACL is equiangular with the triangle AMF; therefore, proportionally, as LC is to CA, so is MF to FA.

And the doubles of the antecedents may be taken; therefore, as the double of LC is to CA, so is the double of MF to FA.

But, as the double of MF is to FA, so is MF to the half of FA; therefore also, as the double of LC is to CA, so is MF to the half of FA.

And the halves of the consequents may be taken; therefore, as the double of LC is to the half of CA, so is MF to the fourth of FA.

And DC is double of LC, CM is half of CA, and FK a fourth part of FA; therefore, as DC is to CM, so is MF to FK.

Componendo also, as the sum of DC, CM is to CM, so is MK to KF; [:ref:`V.18`] therefore also, as the square on the sum of DC, CM is to the square on CM, so is the square on MK to the square on KF.

And since, when the straight line subtending two sides of the pentagon, as AC, is cut in extreme and mean ratio, the greater segment is equal to the side of the pentagon, that is, to DC, [:ref:`XIII.8`] while the square on the greater segment added to the half of the whole is five times the square on the half of the whole, [:ref:`XIII.1`] and CM is half of the whole AC, therefore the square on DC, CM taken as one straight line is five times the square on CM.

But it was proved that, as the square on DC, CM taken as one straight line is to the square on CM, so is the square on MK to the square on KF; therefore the square on MK is five times the square on KF.

But the square on KF is rational, for the diameter is rational; therefore the square on MK is also rational; therefore MK is rational

And, since BF is quadruple of FK, therefore BK is five times KF; therefore the square on BK is twenty-five times the square on KF.

But the square on MK is five times the square on KF; therefore the square on BK is five times the square on KM; therefore the square on BK has not to the square on KM the ratio which a square number has to a square number; therefore BK is incommensurable in length with KM. [:ref:`X.9`]

And each of them is rational.

Therefore BK, KM are rational straight lines commensurable in square only.

But, if from a rational straight line there be subtracted a rational straight line which is commensurable with the whole in square only, the remainder is irrational, namely an apotome; therefore MB is an apotome and MK the annex to it. [:ref:`X.73`]

I say next that MB is also a fourth apotome.

Let the square on N be equal to that by which the square on BK is greater than the square on KM; therefore the square on BK is greater than the square on KM by the square on N.

And, since KF is commensurable with FB, componendo also, KB is commensurable with FB. [:ref:`X.15`]

But BF is commensurable with BH; therefore BK is also commensurable with BH. [:ref:`X.12`]

And, since the square on BK is five times the square on KM, therefore the square on BK has to the square on KM the ratio which 5 has to 1.

Therefore, convertendo, the square on BK has to the square on N the ratio which 5 has to 4 [:ref:`V.19.p.1`], and this is not the ratio which a square number has to a square number; therefore BK is incommensurable with N; [:ref:`X.9`] therefore the square on BK is greater than the square on KM by the square on a straight line incommensurable with BK.

Since then the square on the whole BK is greater than the square on the annex KM by the square on a straight line incommensurable with BK, and the whole BK is commensurable with the rational straight line, BH, set out, therefore MB is a fourth apotome. [:ref:`X.def.3.4`]

But the rectangle contained by a rational straight line and a fourth apotome is irrational, and its square root is irrational, and is called minor. [:ref:`X.94`]

But the square on AB is equal to the rectangle HB, BM, because, when AH is joined, the triangle ABH is equiangular with the triangle ABM, and, as HB is to BA, so is AB to BM.

Therefore the side AB of the pentagon is the irrational straight line called minor. Q. E. D.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "X.15" [URL="/heath/X/15/", target="_top"];
     "elem.7.def.20 elem.7.13" [style="rounded,filled", fillcolor=orange];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "III.24" [URL="/heath/III/24/", target="_top"];
     "X.9" [URL="/heath/X/9/", target="_top"];
     "IV.1" [style="rounded,filled", fillcolor=orange, URL="/heath/IV/1/", target="_top"];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "V.14" [URL="/heath/V/14/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "III.22" [URL="/heath/III/22/", target="_top"];
     "V.4" [URL="/heath/V/4/", target="_top"];
     "X.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.4/", target="_top"];
     "VII.35" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/35/", target="_top"];
     "VI.14" [URL="/heath/VI/14/", target="_top"];
     "I.39" [URL="/heath/I/39/", target="_top"];
     "I.14" [URL="/heath/I/14/", target="_top"];
     "VI.28" [URL="/heath/VI/28/", target="_top"];
     "V.17" [URL="/heath/V/17/", target="_top"];
     "VII.2.p.1" [style="rounded,filled", fillcolor=orange];
     "X.21" [URL="/heath/X/21/", target="_top"];
     "VII.15" [URL="/heath/VII/15/", target="_top"];
     "VI.25" [URL="/heath/VI/25/", target="_top"];
     "X.19" [URL="/heath/X/19/", target="_top"];
     "X.33" [URL="/heath/X/33/", target="_top"];
     "V.21" [URL="/heath/V/21/", target="_top"];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "XIII.8" [URL="/heath/XIII/8/", target="_top"];
     "X.23.p.1" [style="rounded,filled", fillcolor=orange];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "I.45" [URL="/heath/I/45/", target="_top"];
     "VII.22" [URL="/heath/VII/22/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "I.38" [URL="/heath/I/38/", target="_top"];
     "VII.23" [URL="/heath/VII/23/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "elem.7.13 elem.7.def.20" [style="rounded,filled", fillcolor=orange];
     "VI.21" [URL="/heath/VI/21/", target="_top"];
     "I.44" [URL="/heath/I/44/", target="_top"];
     "V.16" [URL="/heath/V/16/", target="_top"];
     "VII.33" [URL="/heath/VII/33/", target="_top"];
     "VII.9" [URL="/heath/VII/9/", target="_top"];
     "I.47" [URL="/heath/I/47/", target="_top"];
     "X.8" [URL="/heath/X/8/", target="_top"];
     "X.16.l.1" [style="rounded,filled", fillcolor=orange];
     "VI.13" [URL="/heath/VI/13/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "VI.20.p.1" [style="rounded,filled", fillcolor=orange];
     "VII.19" [URL="/heath/VII/19/", target="_top"];
     "elem.10.13 elem.10.16" [style="rounded,filled", fillcolor=orange];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "VII.27" [URL="/heath/VII/27/", target="_top"];
     "X.5" [URL="/heath/X/5/", target="_top"];
     "V.18" [URL="/heath/V/18/", target="_top"];
     "VII.1" [URL="/heath/VII/1/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "X.7" [URL="/heath/X/7/", target="_top"];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "III.1.p.1" [style="rounded,filled", fillcolor=orange];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "V.19.p.1" [style="rounded,filled", fillcolor=orange];
     "VI.17" [URL="/heath/VI/17/", target="_top"];
     "V.15" [URL="/heath/V/15/", target="_top"];
     "VIII.26" [URL="/heath/VIII/26/", target="_top"];
     "VII.def.21" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.21/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "VII.12" [URL="/heath/VII/12/", target="_top"];
     "VII.34" [URL="/heath/VII/34/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "VI.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/VI/def.1/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "VIII.2" [URL="/heath/VIII/2/", target="_top"];
     "VII.16" [URL="/heath/VII/16/", target="_top"];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "V.7.p.1" [style="rounded,filled", fillcolor=orange];
     "VII.26" [URL="/heath/VII/26/", target="_top"];
     "III.20" [URL="/heath/III/20/", target="_top"];
     "X.6" [URL="/heath/X/6/", target="_top"];
     "I.43" [URL="/heath/I/43/", target="_top"];
     "I.30" [URL="/heath/I/30/", target="_top"];
     "IV.14" [URL="/heath/IV/14/", target="_top"];
     "VII.13" [URL="/heath/VII/13/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "VIII.4" [URL="/heath/VIII/4/", target="_top"];
     "VIII.18" [URL="/heath/VIII/18/", target="_top"];
     "X.30" [URL="/heath/X/30/", target="_top"];
     "VI.8.p.1" [style="rounded,filled", fillcolor=orange];
     "VI.1" [URL="/heath/VI/1/", target="_top"];
     "XIII.1" [URL="/heath/XIII/1/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "VII.def.20" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.20/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "X.28.l.2" [style="rounded,filled", fillcolor=orange];
     "III.28" [URL="/heath/III/28/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "VII.14" [URL="/heath/VII/14/", target="_top"];
     "V.3" [URL="/heath/V/3/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "X.def.3.4" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.3.4/", target="_top"];
     "X.16" [URL="/heath/X/16/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "I.17" [URL="/heath/I/17/", target="_top"];
     "VII.21" [URL="/heath/VII/21/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "VI.24" [URL="/heath/VI/24/", target="_top"];
     "VII.def.12" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.12/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "V.def.9" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.9/", target="_top"];
     "I.46" [URL="/heath/I/46/", target="_top"];
     "VII.20" [URL="/heath/VII/20/", target="_top"];
     "VI.4" [URL="/heath/VI/4/", target="_top"];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "X.6.p.1" [style="rounded,filled", fillcolor=orange];
     "X.18" [URL="/heath/X/18/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "I.32" [URL="/heath/I/32/", target="_top"];
     "X.11" [URL="/heath/X/11/", target="_top"];
     "elem.7.5 elem.7.6" [style="rounded,filled", fillcolor=orange];
     "VII.18" [URL="/heath/VII/18/", target="_top"];
     "X.12" [URL="/heath/X/12/", target="_top"];
     "VI.26" [URL="/heath/VI/26/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "III.def.11" [style="rounded,filled", fillcolor=orange, URL="/heath/III/def.11/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "XIII.11" [style="rounded,filled", fillcolor=lightblue, URL="/heath/XIII/11/", target="_top"];
     "I.42" [URL="/heath/I/42/", target="_top"];
     "VI.def.3" [style="rounded,filled", fillcolor=orange, URL="/heath/VI/def.3/", target="_top"];
     "VII.10" [URL="/heath/VII/10/", target="_top"];
     "III.10" [URL="/heath/III/10/", target="_top"];
     "X.76" [URL="/heath/X/76/", target="_top"];
     "VII.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.15/", target="_top"];
     "VII.17" [URL="/heath/VII/17/", target="_top"];
     "II.7" [URL="/heath/II/7/", target="_top"];
     "VIII.11" [URL="/heath/VIII/11/", target="_top"];
     "VIII.2.p.1" [style="rounded,filled", fillcolor=orange];
     "V.12" [URL="/heath/V/12/", target="_top"];
     "VII.2" [URL="/heath/VII/2/", target="_top"];
     "III.5" [URL="/heath/III/5/", target="_top"];
     "V.2" [style="rounded,filled", fillcolor=orange, URL="/heath/V/2/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "X.73" [URL="/heath/X/73/", target="_top"];
     "III.26" [URL="/heath/III/26/", target="_top"];
     "III.27" [URL="/heath/III/27/", target="_top"];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "X.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.1/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "X.13" [URL="/heath/X/13/", target="_top"];
     "VI.19.p.1" [style="rounded,filled", fillcolor=orange];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "VII.25" [URL="/heath/VII/25/", target="_top"];
     "VIII.1" [URL="/heath/VIII/1/", target="_top"];
     "VI.33" [URL="/heath/VI/33/", target="_top"];
     "III.31" [URL="/heath/III/31/", target="_top"];
     "VII.4" [URL="/heath/VII/4/", target="_top"];
     "VI.18" [URL="/heath/VI/18/", target="_top"];
     "V.22" [URL="/heath/V/22/", target="_top"];
     "X.94" [URL="/heath/X/94/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "VII.3" [URL="/heath/VII/3/", target="_top"];
     "I.28" [URL="/heath/I/28/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "VI.2" [URL="/heath/VI/2/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "VII.24" [URL="/heath/VII/24/", target="_top"];
     "III.21" [URL="/heath/III/21/", target="_top"];
     "I.6" [style="rounded,filled", fillcolor=orange, URL="/heath/I/6/", target="_top"];
     "VI.16" [URL="/heath/VI/16/", target="_top"];
     "X.73" -> "X.15";
     "XIII.11" -> "X.15";
     "VIII.4" -> "elem.7.def.20 elem.7.13";
     "I.10" -> "I.9";
     "III.26" -> "III.24";
     "X.30" -> "X.9";
     "XIII.11" -> "X.9";
     "X.13" -> "IV.1";
     "V.16" -> "elem.5.17 elem.5.18";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "V.16" -> "V.14";
     "V.18" -> "V.14";
     "XIII.8" -> "V.14";
     "VI.2" -> "V.9";
     "VI.14" -> "V.9";
     "VI.26" -> "V.9";
     "VII.19" -> "V.9";
     "X.6" -> "V.9";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.39" -> "I.31";
     "I.42" -> "I.31";
     "I.44" -> "I.31";
     "I.46" -> "I.31";
     "VI.26" -> "I.31";
     "III.31" -> "III.22";
     "V.22" -> "V.4";
     "X.19" -> "X.def.4";
     "X.21" -> "X.def.4";
     "X.73" -> "X.def.4";
     "VIII.4" -> "VII.35";
     "VI.16" -> "VI.14";
     "VI.2" -> "I.39";
     "I.45" -> "I.14";
     "I.47" -> "I.14";
     "VI.14" -> "I.14";
     "X.33" -> "VI.28";
     "V.18" -> "V.17";
     "VII.3" -> "VII.2.p.1";
     "X.33" -> "X.21";
     "X.94" -> "X.21";
     "VII.16" -> "VII.15";
     "VI.28" -> "VI.25";
     "X.94" -> "X.19";
     "X.76" -> "X.33";
     "V.16" -> "V.21";
     "I.3" -> "I.2";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.28" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "XIII.11" -> "XIII.8";
     "X.33" -> "X.23.p.1";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "III.24" -> "I.8";
     "III.28" -> "I.8";
     "VI.25" -> "I.45";
     "VIII.2" -> "VII.22";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "I.42" -> "I.38";
     "VI.1" -> "I.38";
     "VI.2" -> "I.38";
     "VII.24" -> "VII.23";
     "I.16" -> "I.15";
     "I.28" -> "I.15";
     "I.29" -> "I.15";
     "I.44" -> "I.15";
     "I.16" -> "I.10";
     "I.27" -> "I.def.23";
     "VIII.4" -> "elem.7.13 elem.7.def.20";
     "VI.24" -> "VI.21";
     "VI.28" -> "VI.21";
     "I.45" -> "I.44";
     "VI.25" -> "I.44";
     "VI.1" -> "V.16";
     "VI.4" -> "V.16";
     "VI.24" -> "V.16";
     "VI.25" -> "V.16";
     "VII.34" -> "VII.33";
     "VIII.26" -> "VII.33";
     "VII.10" -> "VII.9";
     "X.13" -> "I.47";
     "X.30" -> "I.47";
     "X.33" -> "I.47";
     "X.11" -> "X.8";
     "X.18" -> "X.16.l.1";
     "VI.25" -> "VI.13";
     "I.28" -> "I.27";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "X.9" -> "VI.20.p.1";
     "VII.24" -> "VII.19";
     "VII.33" -> "VII.19";
     "VII.34" -> "VII.19";
     "X.73" -> "elem.10.13 elem.10.16";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.30" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.39" -> "I.cn.1";
     "I.44" -> "I.cn.1";
     "I.45" -> "I.cn.1";
     "VIII.2" -> "VII.27";
     "X.8" -> "X.5";
     "X.9" -> "X.5";
     "X.11" -> "X.5";
     "X.12" -> "X.5";
     "VI.24" -> "V.18";
     "XIII.11" -> "V.18";
     "VII.2" -> "VII.1";
     "I.17" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "X.11" -> "X.7";
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "V.17" -> "V.1";
     "III.10" -> "III.1.p.1";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.43" -> "I.cn.2";
     "I.45" -> "I.cn.2";
     "I.47" -> "I.cn.2";
     "X.30" -> "V.19.p.1";
     "XIII.11" -> "V.19.p.1";
     "X.94" -> "VI.17";
     "XIII.1" -> "VI.17";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "X.9" -> "VIII.26";
     "VIII.18" -> "VII.def.21";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
     "I.31" -> "I.23";
     "I.42" -> "I.23";
     "III.27" -> "I.23";
     "VI.18" -> "I.23";
     "VII.15" -> "VII.12";
     "VII.20" -> "VII.12";
     "VIII.4" -> "VII.34";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "VI.18" -> "VI.def.1";
     "VI.21" -> "VI.def.1";
     "VI.24" -> "VI.def.1";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "III.5" -> "I.def.15";
     "III.10" -> "I.def.15";
     "VIII.26" -> "VIII.2";
     "VII.18" -> "VII.16";
     "VII.21" -> "VII.16";
     "VII.24" -> "VII.16";
     "VII.33" -> "VII.16";
     "VII.34" -> "VII.16";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.2" -> "V.7";
     "VI.14" -> "V.7";
     "VII.19" -> "V.7";
     "X.5" -> "V.7.p.1";
     "X.6" -> "V.7.p.1";
     "VII.27" -> "VII.26";
     "III.21" -> "III.20";
     "III.27" -> "III.20";
     "X.7" -> "X.6";
     "X.9" -> "X.6";
     "X.11" -> "X.6";
     "X.12" -> "X.6";
     "X.18" -> "X.6";
     "X.73" -> "X.6";
     "I.44" -> "I.43";
     "II.7" -> "I.43";
     "I.45" -> "I.30";
     "XIII.8" -> "IV.14";
     "VII.14" -> "VII.13";
     "VII.17" -> "VII.13";
     "VII.20" -> "VII.13";
     "VIII.4" -> "VII.13";
     "VIII.18" -> "VII.13";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "III.31" -> "I.def.10";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "X.12" -> "VIII.4";
     "VIII.26" -> "VIII.18";
     "X.33" -> "X.30";
     "VI.13" -> "VI.8.p.1";
     "VI.2" -> "VI.1";
     "VI.14" -> "VI.1";
     "VI.25" -> "VI.1";
     "X.19" -> "VI.1";
     "X.21" -> "VI.1";
     "X.94" -> "VI.1";
     "XIII.1" -> "VI.1";
     "XIII.11" -> "XIII.1";
     "V.16" -> "V.11";
     "V.18" -> "V.11";
     "VI.1" -> "V.11";
     "VI.2" -> "V.11";
     "VI.14" -> "V.11";
     "VI.26" -> "V.11";
     "X.6" -> "V.11";
     "X.12" -> "V.11";
     "X.94" -> "V.11";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "I.43" -> "I.34";
     "I.45" -> "I.34";
     "I.46" -> "I.34";
     "VI.4" -> "I.34";
     "VII.12" -> "VII.def.20";
     "VII.13" -> "VII.def.20";
     "VII.17" -> "VII.def.20";
     "VII.20" -> "VII.def.20";
     "VII.33" -> "VII.def.20";
     "X.5" -> "VII.def.20";
     "X.6" -> "VII.def.20";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "V.8" -> "V.def.4";
     "X.30" -> "X.28.l.2";
     "XIII.8" -> "III.28";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "VIII.1" -> "VII.14";
     "V.4" -> "V.3";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.47" -> "I.4";
     "III.26" -> "I.4";
     "XIII.8" -> "I.4";
     "X.94" -> "X.def.3.4";
     "XIII.11" -> "X.def.3.4";
     "X.18" -> "X.16";
     "X.76" -> "X.16";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "III.31" -> "I.17";
     "VI.4" -> "I.17";
     "VII.24" -> "VII.21";
     "VII.33" -> "VII.21";
     "VII.34" -> "VII.21";
     "VIII.1" -> "VII.21";
     "I.30" -> "I.29";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.44" -> "I.29";
     "I.45" -> "I.29";
     "I.46" -> "I.29";
     "VI.26" -> "VI.24";
     "VII.1" -> "VII.def.12";
     "VII.21" -> "VII.def.12";
     "VII.23" -> "VII.def.12";
     "VII.24" -> "VII.def.12";
     "I.13" -> "I.11";
     "I.46" -> "I.11";
     "VIII.11" -> "V.def.9";
     "VIII.18" -> "V.def.9";
     "I.47" -> "I.46";
     "II.7" -> "I.46";
     "VII.21" -> "VII.20";
     "VII.24" -> "VII.20";
     "VII.34" -> "VII.20";
     "VIII.1" -> "VII.20";
     "VIII.4" -> "VII.20";
     "VI.18" -> "VI.4";
     "XIII.8" -> "VI.4";
     "I.34" -> "I.26";
     "X.30" -> "X.6.p.1";
     "X.33" -> "X.18";
     "X.94" -> "X.18";
     "I.36" -> "I.33";
     "I.45" -> "I.33";
     "III.20" -> "I.32";
     "III.22" -> "I.32";
     "III.31" -> "I.32";
     "VI.18" -> "I.32";
     "XIII.8" -> "I.32";
     "XIII.11" -> "I.32";
     "X.19" -> "X.11";
     "X.21" -> "X.11";
     "X.73" -> "X.11";
     "X.94" -> "X.11";
     "VII.9" -> "elem.7.5 elem.7.6";
     "VII.10" -> "elem.7.5 elem.7.6";
     "VII.12" -> "elem.7.5 elem.7.6";
     "VII.19" -> "VII.18";
     "VIII.2" -> "VII.18";
     "VIII.11" -> "VII.18";
     "X.13" -> "X.12";
     "XIII.11" -> "X.12";
     "VI.28" -> "VI.26";
     "X.94" -> "VI.26";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "III.26" -> "III.def.11";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "VI.33" -> "V.def.5";
     "I.44" -> "I.42";
     "I.45" -> "I.42";
     "XIII.1" -> "VI.def.3";
     "VII.13" -> "VII.10";
     "III.24" -> "III.10";
     "X.94" -> "X.76";
     "VII.22" -> "VII.def.15";
     "VII.24" -> "VII.def.15";
     "VII.33" -> "VII.def.15";
     "VII.34" -> "VII.def.15";
     "VII.18" -> "VII.17";
     "VII.19" -> "VII.17";
     "VII.22" -> "VII.17";
     "VII.34" -> "VII.17";
     "VIII.2" -> "VII.17";
     "VIII.11" -> "VII.17";
     "VIII.18" -> "VII.17";
     "X.73" -> "II.7";
     "X.76" -> "II.7";
     "X.9" -> "VIII.11";
     "VIII.26" -> "VIII.2.p.1";
     "V.15" -> "V.12";
     "VII.3" -> "VII.2";
     "VII.4" -> "VII.2";
     "III.10" -> "III.5";
     "V.3" -> "V.2";
     "V.17" -> "V.2";
     "I.8" -> "I.7";
     "XIII.11" -> "X.73";
     "III.27" -> "III.26";
     "III.28" -> "III.26";
     "VI.33" -> "III.27";
     "I.38" -> "I.36";
     "VI.28" -> "I.36";
     "X.15" -> "X.def.1";
     "X.16" -> "X.def.1";
     "I.7" -> "I.5";
     "III.20" -> "I.5";
     "III.31" -> "I.5";
     "XIII.8" -> "I.5";
     "I.39" -> "I.37";
     "I.41" -> "I.37";
     "X.18" -> "X.13";
     "VI.25" -> "VI.19.p.1";
     "X.6" -> "VI.19.p.1";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.43" -> "I.cn.3";
     "VII.27" -> "VII.25";
     "VIII.2" -> "VIII.1";
     "XIII.8" -> "VI.33";
     "VI.13" -> "III.31";
     "X.13" -> "III.31";
     "X.30" -> "III.31";
     "VII.20" -> "VII.4";
     "VI.25" -> "VI.18";
     "VI.28" -> "VI.18";
     "VI.4" -> "V.22";
     "VI.24" -> "V.22";
     "X.5" -> "V.22";
     "X.6" -> "V.22";
     "X.12" -> "V.22";
     "XIII.11" -> "X.94";
     "I.29" -> "I.post.5";
     "I.44" -> "I.post.5";
     "VI.4" -> "I.post.5";
     "VII.33" -> "VII.3";
     "VI.4" -> "I.28";
     "I.4" -> "I.cn.4";
     "VI.4" -> "VI.2";
     "VI.24" -> "VI.2";
     "I.42" -> "I.41";
     "I.47" -> "I.41";
     "VI.1" -> "I.41";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "VII.25" -> "VII.24";
     "VII.26" -> "VII.24";
     "III.22" -> "III.21";
     "IV.14" -> "I.6";
     "XIII.8" -> "I.6";
     "VI.17" -> "VI.16";
   }



Required for
------------

:ref:`XIII.16`, :ref:`XIII.18`