...
:number: 288
:type: prop
:dependencies: VII.13, VII.def.20




.. figure:: VII.17.graphic.inverted.png
...
Therefore, as the unit F is to the number A, so is B to D. [:ref:`VII.def.20`]

For the same reason, as the unit F is to the number A, so also is C to E; therefore also, as B is to D, so is C to E.

Therefore, alternately, as B is to C, so is D to E. [:ref:`VII.13`] Q. E. D.
...
.. graphviz::
   ...
   digraph {
     ...
     "VII.17" [style="rounded,filled", fillcolor=lightblue, URL="/heath/VII/17/", target="_top"];
     "VII.13" -> "VII.def.20";
     "VII.17" -> "VII.def.20";
