:order: 39
:number: 310
:type: prop
:dependencies: VII.36, VII.37, VII.38




.. figure:: VII.39.graphic.inverted.png

.. _VII.39:

VII.39
======

   To find the number which is the least that will have given parts.

Let A, B, C be the given parts; thus it is required to find the number which is the least that will have the parts A, B, C.

Let D, E, F be numbers called by the same name as the parts A, B, C, and let G, the least number measured by D, E, F, be taken. [:ref:`VII.36`]

Therefore G has parts called by the same name as D, E, F. [:ref:`VII.37`]

But A, B, C are parts called by the same name as D, E, F; therefore G has the parts A, B, C.

I say next that it is also the least number that has.

For, if not, there will be some number less than G which will have the parts A, B, C.

Let it be H.

Since H has the parts A, B, C, therefore H will be measured by numbers called by the same name as the parts A, B, C. [:ref:`VII.38`]

But D, E, F are numbers called by the same name as the parts A, B, C; therefore H is measured by D, E, F.

And it is less than G : which is impossible.

Therefore there will be no number less than G that will have the parts A, B, C. Q. E. D.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "VII.36" [URL="/heath/VII/36/", target="_top"];
     "VII.1" [URL="/heath/VII/1/", target="_top"];
     "VII.9" [URL="/heath/VII/9/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "VII.17" [URL="/heath/VII/17/", target="_top"];
     "VII.3" [URL="/heath/VII/3/", target="_top"];
     "elem.7.5 elem.7.6" [style="rounded,filled", fillcolor=orange];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "VII.13" [URL="/heath/VII/13/", target="_top"];
     "VII.2" [URL="/heath/VII/2/", target="_top"];
     "VII.12" [URL="/heath/VII/12/", target="_top"];
     "VII.def.12" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.12/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "VII.33" [URL="/heath/VII/33/", target="_top"];
     "elem.7.2.p.1" [style="rounded,filled", fillcolor=orange];
     "VII.10" [URL="/heath/VII/10/", target="_top"];
     "VII.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.15/", target="_top"];
     "VII.19" [URL="/heath/VII/19/", target="_top"];
     "VII.18" [URL="/heath/VII/18/", target="_top"];
     "VII.39" [style="rounded,filled", fillcolor=lightblue, URL="/heath/VII/39/", target="_top"];
     "VII.16" [URL="/heath/VII/16/", target="_top"];
     "VII.4" [URL="/heath/VII/4/", target="_top"];
     "VII.21" [URL="/heath/VII/21/", target="_top"];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "VII.38" [URL="/heath/VII/38/", target="_top"];
     "VII.35" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/35/", target="_top"];
     "VII.37" [URL="/heath/VII/37/", target="_top"];
     "VII.20" [URL="/heath/VII/20/", target="_top"];
     "VII.34" [URL="/heath/VII/34/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "VII.15" [URL="/heath/VII/15/", target="_top"];
     "VII.def.20" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.20/", target="_top"];
     "VII.39" -> "VII.36";
     "VII.2" -> "VII.1";
     "VII.10" -> "VII.9";
     "V.7" -> "V.def.5";
     "VII.18" -> "VII.17";
     "VII.19" -> "VII.17";
     "VII.34" -> "VII.17";
     "VII.33" -> "VII.3";
     "VII.9" -> "elem.7.5 elem.7.6";
     "VII.10" -> "elem.7.5 elem.7.6";
     "VII.12" -> "elem.7.5 elem.7.6";
     "VII.19" -> "V.7";
     "VII.19" -> "V.9";
     "VII.17" -> "VII.13";
     "VII.20" -> "VII.13";
     "VII.3" -> "VII.2";
     "VII.4" -> "VII.2";
     "VII.15" -> "VII.12";
     "VII.20" -> "VII.12";
     "VII.1" -> "VII.def.12";
     "VII.21" -> "VII.def.12";
     "V.9" -> "V.8";
     "VII.34" -> "VII.33";
     "VII.3" -> "elem.7.2.p.1";
     "VII.13" -> "VII.10";
     "VII.33" -> "VII.def.15";
     "VII.34" -> "VII.def.15";
     "VII.33" -> "VII.19";
     "VII.34" -> "VII.19";
     "VII.19" -> "VII.18";
     "VII.18" -> "VII.16";
     "VII.21" -> "VII.16";
     "VII.33" -> "VII.16";
     "VII.34" -> "VII.16";
     "VII.20" -> "VII.4";
     "VII.33" -> "VII.21";
     "VII.34" -> "VII.21";
     "V.8" -> "V.1";
     "VII.39" -> "VII.38";
     "VII.36" -> "VII.35";
     "VII.39" -> "VII.37";
     "VII.21" -> "VII.20";
     "VII.34" -> "VII.20";
     "VII.36" -> "VII.34";
     "V.8" -> "V.def.4";
     "V.8" -> "V.def.7";
     "VII.16" -> "VII.15";
     "VII.37" -> "VII.15";
     "VII.38" -> "VII.15";
     "VII.12" -> "VII.def.20";
     "VII.13" -> "VII.def.20";
     "VII.17" -> "VII.def.20";
     "VII.20" -> "VII.def.20";
     "VII.33" -> "VII.def.20";
   }
