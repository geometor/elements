...
And, since ``BC`` is equal to ``BD``, and ``GK`` to ``KN``, therefore ``CK`` is also equal to ``KD``, and ``GR`` to ``RN``. [:ref:`I.36`]

But ``CK`` is equal to ``RN``, for they are complements of the parallelogram ``CP``; [:ref:`I.43`] therefore ``KD`` is also equal to ``GR``; therefore the four areas ``DK``, ``CK``, ``GR``, ``RN`` are equal to one another.
...
.. container:: center
   ...
   ``AG`` is also equal to ``MQ``, and ``QL`` to ``RF``. [:ref:`I.36`]


But ``MQ`` is equal to ``QL``, for they are complements of the parallelogram ``ML``; [:ref:`I.43`]
...
.. graphviz::
   ...
   digraph {
     ...
     "II.8" [style="rounded,filled", fillcolor=lightblue, URL="/heath/II/8/", target="_top"];
     "I.2" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.43" -> "I.cn.3";
