...
Then, since, as ``BC`` is to ``GH``, so is ``GH`` to ``CF``, and, if three straight lines be proportional, as the first is to the third, so is the figure on the first to the similar and similarly situated figure described on the second, [:ref:`VI.19.p.1`] therefore, as ``BC`` is to ``CF``, so is the triangle ``ABC`` to the triangle ``KGH``.
