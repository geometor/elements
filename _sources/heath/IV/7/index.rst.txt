...
.. figure:: IV.7.graphic.inverted.png
...
Let two diameters ``AC``, ``BD`` of the circle ``ABCD`` be drawn at right angles to one another, and through the points ``A``, ``B``, ``C``, ``D`` let ``FG``, ``GH``, ``HK``, ``KF`` be drawn touching the circle ``ABCD``. [:ref:`III.16.p.1`]
...
.. graphviz::
   ...
   digraph {
     ...
     "IV.7" [style="rounded,filled", fillcolor=lightblue, URL="/heath/IV/7/", target="_top"];
