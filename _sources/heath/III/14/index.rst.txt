...
But the squares on ``AF``, ``EF`` are equal to the square on ``AE``, for the angle at ``F`` is right; and the squares on ``EG``, ``GC`` are equal to the square on ``EC``, for the angle at ``G`` is right; [:ref:`I.47`]
...
But in a circle straight lines are said to be equally distant from the centre when the perpendiculars drawn to them from the centre are equal; [:ref:`III.def.4`]
...
But the squares on ``EF``, ``FA`` are equal to the square on ``AE``, and the squares on ``EG``, ``GC`` equal to the square on ``CE``. [:ref:`I.47`]
...
.. graphviz::
   ...
   digraph {
     ...
     "III.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/III/def.4/", target="_top"];
     ...
     "III.14" [style="rounded,filled", fillcolor=lightblue, URL="/heath/III/14/", target="_top"];
