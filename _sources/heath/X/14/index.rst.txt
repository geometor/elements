:order: 14
:number: 391
:type: None
:tags: line
:dependencies: V.17, V.22, VI.22, X.11




.. figure:: X.14.graphic.inverted.png

.. _X.14:

X.14
====

   If four straight lines be proportional, and the square on the first be greater than the square on the second by the square on a straight line commensurable with the first, the square on the third will also be greater than the square on the fourth by
          the square on a straight line commensurable with the third.

And, if the square on the first be greater than the square on the second by the square on a straight line incommensurable with the first, the square on the third will also be greater than the square on the fourth by the square on a straight line in-
commensurable with the third.

Let A, B, C, D be four straight lines in proportion, so that, as A is to B, so is C to D; and let the square on A be greater than the square on B by the square on E, and let the square on C be greater than the square on D by the square on F; I say that, if A is commensurable with E, C is also commensurable with F, and, if A is incommensurable with E, C is also incommensurable with F.

For since, as A is to B, so is C to D, therefore also, as the square on A is to the square on B, so is the square on C to the square on D. [:ref:`VI.22`]

But the squares on E, B are equal to the square on A, and the squares on D, F are equal to the square on C.

Therefore, as the squares on E, B are to the square on B, so are the squares on D, F to the square on D; therefore, separando, as the square on E is to the square on B, so is the square on F to the square on D; [:ref:`V.17`] therefore also, as E is to B, so is F to D; [:ref:`VI.22`] therefore, inversely, as B is to E, so is D to F.

But, as A is to B, so also is C to D; therefore, ex aequali, as A is to E, so is C to F. [:ref:`V.22`]

Therefore, if A is commensurable with E, C is also commensurable with F, and, if A is incommensurable with E, C is also incommensurable with F. [:ref:`X.11`]

Therefore etc.3, 5, 8, 10. Euclid speaks of the square on the first (third) being greater than the square on the second (fourth) by the square on a straight line commensurable (incommensurable) with itself (ἑαυτῇ),
and similarly in all like phrases throughout the Book. For clearness' sake I substitute the first,
the third,
or whatever it may be, for itself
in these cases.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "X.7" [URL="/heath/X/7/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "V.2" [style="rounded,filled", fillcolor=orange, URL="/heath/V/2/", target="_top"];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "V.22" [URL="/heath/V/22/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "elem.5.7.p.1" [style="rounded,filled", fillcolor=orange];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "V.4" [URL="/heath/V/4/", target="_top"];
     "I.28" [URL="/heath/I/28/", target="_top"];
     "VI.4" [URL="/heath/VI/4/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "X.8" [URL="/heath/X/8/", target="_top"];
     "VI.2" [URL="/heath/VI/2/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "X.6" [URL="/heath/X/6/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "VI.22" [URL="/heath/VI/22/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "VI.12" [URL="/heath/VI/12/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "VI.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/VI/def.1/", target="_top"];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "V.17" [URL="/heath/V/17/", target="_top"];
     "V.3" [URL="/heath/V/3/", target="_top"];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "V.12" [URL="/heath/V/12/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "I.17" [URL="/heath/I/17/", target="_top"];
     "I.32" [URL="/heath/I/32/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "X.14" [style="rounded,filled", fillcolor=lightblue, URL="/heath/X/14/", target="_top"];
     "I.38" [URL="/heath/I/38/", target="_top"];
     "V.16" [URL="/heath/V/16/", target="_top"];
     "V.15" [URL="/heath/V/15/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "V.21" [URL="/heath/V/21/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "VI.18" [URL="/heath/VI/18/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "VI.11" [URL="/heath/VI/11/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "X.5" [URL="/heath/X/5/", target="_top"];
     "VII.def.20" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.20/", target="_top"];
     "elem.6.19.p.1" [style="rounded,filled", fillcolor=orange];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "X.11" [URL="/heath/X/11/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "I.39" [URL="/heath/I/39/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "VI.1" [URL="/heath/VI/1/", target="_top"];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "V.14" [URL="/heath/V/14/", target="_top"];
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "V.17" -> "V.1";
     "X.11" -> "X.7";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.8" -> "I.7";
     "I.28" -> "I.27";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "I.27" -> "I.def.23";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "VI.4" -> "I.34";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "V.3" -> "V.2";
     "V.17" -> "V.2";
     "I.10" -> "I.9";
     "VI.4" -> "V.22";
     "VI.22" -> "V.22";
     "X.5" -> "V.22";
     "X.6" -> "V.22";
     "X.14" -> "V.22";
     "I.15" -> "I.post.4";
     "X.5" -> "elem.5.7.p.1";
     "X.6" -> "elem.5.7.p.1";
     "I.3" -> "I.2";
     "V.22" -> "V.4";
     "VI.4" -> "I.28";
     "VI.18" -> "VI.4";
     "I.4" -> "I.cn.4";
     "X.11" -> "X.8";
     "VI.4" -> "VI.2";
     "VI.11" -> "VI.2";
     "VI.12" -> "VI.2";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
     "I.31" -> "I.23";
     "VI.18" -> "I.23";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.28" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "I.7" -> "I.5";
     "I.13" -> "I.11";
     "X.7" -> "X.6";
     "X.11" -> "X.6";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "X.14" -> "VI.22";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "VI.22" -> "VI.12";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "VI.2" -> "V.9";
     "VI.22" -> "V.9";
     "X.6" -> "V.9";
     "VI.18" -> "VI.def.1";
     "V.16" -> "elem.5.17 elem.5.18";
     "X.14" -> "V.17";
     "V.4" -> "V.3";
     "I.38" -> "I.36";
     "V.15" -> "V.12";
     "I.16" -> "I.15";
     "I.28" -> "I.15";
     "I.29" -> "I.15";
     "VI.4" -> "I.17";
     "VI.18" -> "I.32";
     "VI.1" -> "I.41";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "I.2" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "VI.1" -> "I.38";
     "VI.2" -> "I.38";
     "VI.1" -> "V.16";
     "VI.4" -> "V.16";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "V.8" -> "V.def.4";
     "V.16" -> "V.21";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.2" -> "V.7";
     "VI.22" -> "VI.18";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "VI.22" -> "VI.11";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "X.8" -> "X.5";
     "X.11" -> "X.5";
     "X.5" -> "VII.def.20";
     "X.6" -> "VII.def.20";
     "VI.22" -> "elem.6.19.p.1";
     "X.6" -> "elem.6.19.p.1";
     "I.34" -> "I.26";
     "I.36" -> "I.33";
     "X.14" -> "X.11";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "VI.2" -> "I.39";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "I.16" -> "I.10";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "VI.11" -> "I.3";
     "I.17" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.39" -> "I.31";
     "VI.11" -> "I.31";
     "VI.12" -> "I.31";
     "V.16" -> "V.11";
     "VI.1" -> "V.11";
     "VI.2" -> "V.11";
     "VI.22" -> "V.11";
     "X.6" -> "V.11";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "I.29" -> "I.post.5";
     "VI.4" -> "I.post.5";
     "VI.2" -> "VI.1";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.39" -> "I.cn.1";
     "I.39" -> "I.37";
     "I.41" -> "I.37";
     "V.16" -> "V.14";
   }



Required for
------------

:ref:`X.101`, :ref:`X.102`, :ref:`X.103`, :ref:`X.106`, :ref:`X.107`, :ref:`X.109`, :ref:`X.110`, :ref:`X.112`, :ref:`X.113`, :ref:`X.114`, :ref:`X.31`, :ref:`X.32`, :ref:`X.34`, :ref:`X.35`, :ref:`X.40`, :ref:`X.41`, :ref:`X.46`, :ref:`X.58`, :ref:`X.59`, :ref:`X.64`, :ref:`X.65`, :ref:`X.66`, :ref:`X.69`, :ref:`X.70`, :ref:`X.71`, :ref:`X.72`, :ref:`X.77`, :ref:`X.78`, :ref:`X.83`, :ref:`X.84`, :ref:`X.95`, :ref:`X.96`