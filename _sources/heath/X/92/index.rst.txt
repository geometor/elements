:order: 92
:number: 481
:type: prop
:tags: line
:dependencies: V.11, VI.1, VI.17, VI.26, X.11, X.13, X.15, X.17, X.19, X.21, X.73, X.74, elem.10.def.3.2




.. figure:: X.92.graphic.inverted.png

.. _X.92:

X.92
====

   If an area be contained by a rational straight line and a second apotome, the side
    of the area is a first apotome of a medial straight line.

For let the area AB be contained by the rational straight line AC and the second apotome AD; I say that the side
of the area AB is a first apotome of a medial straight line.

For let DG be the annex to AD; therefore AG, GD are rational straight lines commensurable in square only, [:ref:`X.73`] and the annex DG is commensurable with the rational straight line AC set out, while the square on the whole AG is greater than the square on the annex GD by the square on a straight line commensurable in length with AG. [:ref:`elem.10.def.3.2`]

Since then the square on AG is greater than the square on GD by the square on a straight line commensurable with AG, therefore, if there be applied to AG a parallelogram equal to the fourth part of the square on GD and deficient by a square figure, it divides it into commensurable parts. [:ref:`X.17`]

Let then DG be bisected at E, let there be applied to AG a parallelogram equal to the square on EG and deficient by a square figure, and let it be the rectangle AF, FG; therefore AF is commensurable in length with FG.

Therefore AG is also commensurable in length with each of the straight lines AF, FG. [:ref:`X.15`]

But AG is rational and incommensurable in length with AC; therefore each of the straight lines AF, FG is also rational and incommensurable in length with AC; [:ref:`X.13`] therefore each of the rectangles AI, FK is medial. [:ref:`X.21`]

Again, since DE is commensurable with EG, therefore DG is also commensurable with each of the straight lines DE, EG. [:ref:`X.15`]

But DG is commensurable in length with AC.

Therefore each of the rectangles DH, EK is rational. [:ref:`X.19`]

Let then the square LM be constructed equal to AI, and let there be subtracted NO equal to FK and being about the same angle with LM, namely the angle LPM; therefore the squares LM, NO are about the same diameter. [:ref:`VI.26`]

Let PR be their diameter, and let the figure be drawn.

Since then AI, FK are medial and are equal to the squares on LP, PN, the squares on LP, PN are also medial; therefore LP, PN are also medial straight lines commensurable in square only.

And, since the rectangle AF, FG is equal to the square on EG, therefore, as AF is to EG, so is EG to FG, [:ref:`VI.17`] while, as AF is to EG, so is AI to EK, and, as EG is to FG, so is EK to FK; [:ref:`VI.1`] therefore EK is a mean proportional between AI, FK. [:ref:`V.11`]

But MN is also a mean proportional between the squares LM, NO, and AI is equal to LM, and FK to NO; therefore MN is also equal to EK.

But DH is equal to EK, and LO equal to MN; therefore the whole DK is equal to the gnomon UVW and NO.

Since then the whole AK is equal to LM, NO, and, in these, DK is equal to the gnomon UVW and NO, therefore the remainder AB is equal to TS.

But TS is the square on LN; therefore the square on LN is equal to the area AB; therefore LN is the side
of the area AB.

I say that LN is a first apotome of a medial straight line.

For, since EK is rational and is equal to LO, therefore LO, that is, the rectangle LP, PN, is rational.

But NO was proved medial; therefore LO is incommensurable with NO.

But, as LO is to NO, so is LP to PN; [:ref:`VI.1`] therefore LP, PN are incommensurable in length. [:ref:`X.11`]

Therefore LP, PN are medial straight lines commensurable in square only which contain a rational rectangle; therefore LN is a first apotome of a medial straight line. [:ref:`X.74`]

And it is the side
of the area AB.

Therefore the side
of the area AB is a first apotome of a medial straight line. Q. E. D.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "VII.9" [URL="/heath/VII/9/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "III.31" [URL="/heath/III/31/", target="_top"];
     "X.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.1/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "X.17" [URL="/heath/X/17/", target="_top"];
     "elem.7.5 elem.7.6" [style="rounded,filled", fillcolor=orange];
     "VI.1" [URL="/heath/VI/1/", target="_top"];
     "elem.10.13 elem.10.16" [style="rounded,filled", fillcolor=orange];
     "I.14" [URL="/heath/I/14/", target="_top"];
     "V.16" [URL="/heath/V/16/", target="_top"];
     "VII.13" [URL="/heath/VII/13/", target="_top"];
     "VII.2" [URL="/heath/VII/2/", target="_top"];
     "elem.7.13 elem.7.def.20" [style="rounded,filled", fillcolor=orange];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "VII.def.12" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.12/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "VII.10" [URL="/heath/VII/10/", target="_top"];
     "elem.7.2.p.1" [style="rounded,filled", fillcolor=orange];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "VII.18" [URL="/heath/VII/18/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "VII.20" [URL="/heath/VII/20/", target="_top"];
     "VII.35" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/35/", target="_top"];
     "VI.26" [URL="/heath/VI/26/", target="_top"];
     "X.16" [URL="/heath/X/16/", target="_top"];
     "I.32" [URL="/heath/I/32/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "III.22" [URL="/heath/III/22/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "X.74" [URL="/heath/X/74/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "V.21" [URL="/heath/V/21/", target="_top"];
     "I.39" [URL="/heath/I/39/", target="_top"];
     "V.14" [URL="/heath/V/14/", target="_top"];
     "V.12" [URL="/heath/V/12/", target="_top"];
     "X.73" [URL="/heath/X/73/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "VI.2" [URL="/heath/VI/2/", target="_top"];
     "VII.17" [URL="/heath/VII/17/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "VII.3" [URL="/heath/VII/3/", target="_top"];
     "VII.15" [URL="/heath/VII/15/", target="_top"];
     "II.5" [URL="/heath/II/5/", target="_top"];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "X.92" [style="rounded,filled", fillcolor=lightblue, URL="/heath/X/92/", target="_top"];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "VII.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.15/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "V.17" [URL="/heath/V/17/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "V.4" [URL="/heath/V/4/", target="_top"];
     "VII.4" [URL="/heath/VII/4/", target="_top"];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "VI.24" [URL="/heath/VI/24/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "X.5" [URL="/heath/X/5/", target="_top"];
     "X.11" [URL="/heath/X/11/", target="_top"];
     "VII.34" [URL="/heath/VII/34/", target="_top"];
     "VI.14" [URL="/heath/VI/14/", target="_top"];
     "V.2" [style="rounded,filled", fillcolor=orange, URL="/heath/V/2/", target="_top"];
     "X.15" [URL="/heath/X/15/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "VII.1" [URL="/heath/VII/1/", target="_top"];
     "VI.21" [URL="/heath/VI/21/", target="_top"];
     "III.20" [URL="/heath/III/20/", target="_top"];
     "X.13" [URL="/heath/X/13/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "I.47" [URL="/heath/I/47/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "V.3" [URL="/heath/V/3/", target="_top"];
     "VII.12" [URL="/heath/VII/12/", target="_top"];
     "X.7" [URL="/heath/X/7/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "II.7" [URL="/heath/II/7/", target="_top"];
     "X.19" [URL="/heath/X/19/", target="_top"];
     "IV.1" [style="rounded,filled", fillcolor=orange, URL="/heath/IV/1/", target="_top"];
     "I.17" [URL="/heath/I/17/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "X.21" [URL="/heath/X/21/", target="_top"];
     "VII.21" [URL="/heath/VII/21/", target="_top"];
     "VI.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/VI/def.1/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "I.38" [URL="/heath/I/38/", target="_top"];
     "VII.def.20" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.20/", target="_top"];
     "elem.7.def.20 elem.7.13" [style="rounded,filled", fillcolor=orange];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "V.22" [URL="/heath/V/22/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "X.6" [URL="/heath/X/6/", target="_top"];
     "X.8" [URL="/heath/X/8/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "V.15" [URL="/heath/V/15/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "VI.17" [URL="/heath/VI/17/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "V.18" [URL="/heath/V/18/", target="_top"];
     "I.43" [URL="/heath/I/43/", target="_top"];
     "X.12" [URL="/heath/X/12/", target="_top"];
     "elem.6.19.p.1" [style="rounded,filled", fillcolor=orange];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "I.46" [URL="/heath/I/46/", target="_top"];
     "VII.33" [URL="/heath/VII/33/", target="_top"];
     "VII.19" [URL="/heath/VII/19/", target="_top"];
     "VII.16" [URL="/heath/VII/16/", target="_top"];
     "elem.5.7.p.1" [style="rounded,filled", fillcolor=orange];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "VIII.4" [URL="/heath/VIII/4/", target="_top"];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "VI.16" [URL="/heath/VI/16/", target="_top"];
     "III.21" [URL="/heath/III/21/", target="_top"];
     "elem.10.def.3.2" [style="rounded,filled", fillcolor=orange];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "X.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.4/", target="_top"];
     "VII.10" -> "VII.9";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "X.13" -> "III.31";
     "X.15" -> "X.def.1";
     "X.16" -> "X.def.1";
     "V.16" -> "V.11";
     "V.18" -> "V.11";
     "VI.1" -> "V.11";
     "VI.2" -> "V.11";
     "VI.14" -> "V.11";
     "VI.26" -> "V.11";
     "X.6" -> "V.11";
     "X.12" -> "V.11";
     "X.92" -> "V.11";
     "X.92" -> "X.17";
     "VII.9" -> "elem.7.5 elem.7.6";
     "VII.10" -> "elem.7.5 elem.7.6";
     "VII.12" -> "elem.7.5 elem.7.6";
     "VI.2" -> "VI.1";
     "VI.14" -> "VI.1";
     "X.19" -> "VI.1";
     "X.21" -> "VI.1";
     "X.92" -> "VI.1";
     "X.73" -> "elem.10.13 elem.10.16";
     "I.47" -> "I.14";
     "VI.14" -> "I.14";
     "VI.1" -> "V.16";
     "VI.24" -> "V.16";
     "VII.17" -> "VII.13";
     "VII.20" -> "VII.13";
     "VIII.4" -> "VII.13";
     "VII.3" -> "VII.2";
     "VII.4" -> "VII.2";
     "VIII.4" -> "elem.7.13 elem.7.def.20";
     "I.38" -> "I.36";
     "II.5" -> "I.36";
     "VII.1" -> "VII.def.12";
     "VII.21" -> "VII.def.12";
     "I.39" -> "I.37";
     "I.41" -> "I.37";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "I.43" -> "I.34";
     "I.46" -> "I.34";
     "VII.13" -> "VII.10";
     "VII.3" -> "elem.7.2.p.1";
     "I.3" -> "I.2";
     "I.27" -> "I.def.23";
     "VII.19" -> "VII.18";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "VII.21" -> "VII.20";
     "VII.34" -> "VII.20";
     "VIII.4" -> "VII.20";
     "VIII.4" -> "VII.35";
     "X.92" -> "VI.26";
     "X.74" -> "X.16";
     "III.20" -> "I.32";
     "III.22" -> "I.32";
     "III.31" -> "I.32";
     "V.8" -> "V.def.4";
     "I.16" -> "I.15";
     "I.29" -> "I.15";
     "III.31" -> "III.22";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "X.92" -> "X.74";
     "I.7" -> "I.5";
     "III.20" -> "I.5";
     "III.31" -> "I.5";
     "V.16" -> "V.21";
     "VI.2" -> "I.39";
     "V.16" -> "V.14";
     "V.18" -> "V.14";
     "V.15" -> "V.12";
     "X.92" -> "X.73";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "VI.24" -> "VI.2";
     "VII.18" -> "VII.17";
     "VII.19" -> "VII.17";
     "VII.34" -> "VII.17";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "VII.33" -> "VII.3";
     "VII.16" -> "VII.15";
     "X.17" -> "II.5";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.2" -> "V.7";
     "VI.14" -> "V.7";
     "VII.19" -> "V.7";
     "I.31" -> "I.23";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "I.8" -> "I.7";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "VII.33" -> "VII.def.15";
     "VII.34" -> "VII.def.15";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.47" -> "I.4";
     "V.18" -> "V.17";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "V.22" -> "V.4";
     "VII.20" -> "VII.4";
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "V.17" -> "V.1";
     "VI.26" -> "VI.24";
     "I.17" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "X.8" -> "X.5";
     "X.11" -> "X.5";
     "X.12" -> "X.5";
     "X.19" -> "X.11";
     "X.21" -> "X.11";
     "X.73" -> "X.11";
     "X.92" -> "X.11";
     "VIII.4" -> "VII.34";
     "VI.16" -> "VI.14";
     "V.3" -> "V.2";
     "V.17" -> "V.2";
     "X.17" -> "X.15";
     "X.73" -> "X.15";
     "X.92" -> "X.15";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "III.31" -> "I.def.10";
     "I.13" -> "I.11";
     "I.46" -> "I.11";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "VII.2" -> "VII.1";
     "VI.24" -> "VI.21";
     "III.21" -> "III.20";
     "X.92" -> "X.13";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "X.13" -> "I.47";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.43" -> "I.cn.3";
     "V.4" -> "V.3";
     "VII.15" -> "VII.12";
     "VII.20" -> "VII.12";
     "X.11" -> "X.7";
     "I.47" -> "I.41";
     "VI.1" -> "I.41";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "X.73" -> "II.7";
     "X.74" -> "II.7";
     "X.92" -> "X.19";
     "X.13" -> "IV.1";
     "III.31" -> "I.17";
     "I.36" -> "I.33";
     "X.92" -> "X.21";
     "VII.33" -> "VII.21";
     "VII.34" -> "VII.21";
     "VI.21" -> "VI.def.1";
     "VI.24" -> "VI.def.1";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
     "I.10" -> "I.9";
     "I.4" -> "I.cn.4";
     "VI.1" -> "I.38";
     "VI.2" -> "I.38";
     "VII.12" -> "VII.def.20";
     "VII.13" -> "VII.def.20";
     "VII.17" -> "VII.def.20";
     "VII.20" -> "VII.def.20";
     "VII.33" -> "VII.def.20";
     "X.5" -> "VII.def.20";
     "X.6" -> "VII.def.20";
     "VIII.4" -> "elem.7.def.20 elem.7.13";
     "I.34" -> "I.26";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.46" -> "I.29";
     "VI.24" -> "V.22";
     "X.5" -> "V.22";
     "X.6" -> "V.22";
     "X.12" -> "V.22";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.43" -> "I.cn.2";
     "I.47" -> "I.cn.2";
     "X.7" -> "X.6";
     "X.11" -> "X.6";
     "X.12" -> "X.6";
     "X.17" -> "X.6";
     "X.73" -> "X.6";
     "X.11" -> "X.8";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.39" -> "I.cn.1";
     "X.92" -> "VI.17";
     "VI.2" -> "V.9";
     "VI.14" -> "V.9";
     "VI.26" -> "V.9";
     "VII.19" -> "V.9";
     "X.6" -> "V.9";
     "VI.24" -> "V.18";
     "II.5" -> "I.43";
     "II.7" -> "I.43";
     "X.13" -> "X.12";
     "X.17" -> "X.12";
     "X.6" -> "elem.6.19.p.1";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "I.16" -> "I.10";
     "I.47" -> "I.46";
     "II.5" -> "I.46";
     "II.7" -> "I.46";
     "VII.34" -> "VII.33";
     "VII.33" -> "VII.19";
     "VII.34" -> "VII.19";
     "VII.18" -> "VII.16";
     "VII.21" -> "VII.16";
     "VII.33" -> "VII.16";
     "VII.34" -> "VII.16";
     "X.5" -> "elem.5.7.p.1";
     "X.6" -> "elem.5.7.p.1";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.39" -> "I.31";
     "I.46" -> "I.31";
     "II.5" -> "I.31";
     "VI.26" -> "I.31";
     "X.12" -> "VIII.4";
     "V.16" -> "elem.5.17 elem.5.18";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "VI.17" -> "VI.16";
     "III.22" -> "III.21";
     "X.92" -> "elem.10.def.3.2";
     "I.29" -> "I.post.5";
     "X.19" -> "X.def.4";
     "X.21" -> "X.def.4";
     "X.73" -> "X.def.4";
     "X.74" -> "X.def.4";
   }



Required for
------------

:ref:`X.109`