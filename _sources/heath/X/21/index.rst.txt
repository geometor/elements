:order: 21
:number: 398
:type: prop
:tags: line
:dependencies: VI.1, X.11, X.def.4




.. figure:: X.21.graphic.inverted.png

.. _X.21:

X.21
====

   The rectangle contained by rational straight lines commensurable in square only is irrational, and the side of the square equal to it is irrational. Let the latter be called
          medial.

For let the rectangle AC be contained by the rational straight lines AB, BC commensurable in square only; I say that AC is irrational, and the side of the square equal to it is irrational; and let the latter be called medial.

For on AB let the square AD be described; therefore AD is rational. [:ref:`X.def.4`]

And, since AB is incommensurable in length with BC, for by hypothesis they are commensurable in square only, while AB is equal to BD, therefore DB is also incommensurable in length with BC.

And, as DB is to BC, so is AD to AC; [:ref:`VI.1`] therefore DA is incommensurable with AC. [:ref:`X.11`]

But DA is rational; therefore AC is irrational, so that the side of the square equal to AC is also irrational. [:ref:`X.def.4`]

And let the latter be called medial. Q. E. D.


.. _X.21.l.1:


**X.21.l.1**


If there be two straight lines, then, as the first is to the second, so is the square on the first to the rectangle contained by the two straight lines. 
Let FE, EG be two straight lines. 
       
I say that, as FE is to EG, so is the square on FE to the rectangle FE, EG. 
For on FE let the square DF be described, and let GD be completed. 
Since then, as FE is to EG, so is FD to DG, [:ref:`VI.1`] and FD is the square on FE, and DG the rectangle DE, EG, that is, the rectangle FE, EG, therefore, as FE is to EG, so is the square on FE to the rectangle FE, EG. 
Similarly also, as the rectangle GE, EF is to the square on EF, that is, as GD is to FD, so is GE to EF. Q. E. D.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "V.7.p.1" [style="rounded,filled", fillcolor=orange];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "V.14" [URL="/heath/V/14/", target="_top"];
     "X.6" [URL="/heath/X/6/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "V.4" [URL="/heath/V/4/", target="_top"];
     "X.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.4/", target="_top"];
     "VI.1" [URL="/heath/VI/1/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "VII.def.20" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.20/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "X.21" [style="rounded,filled", fillcolor=lightblue, URL="/heath/X/21/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "V.21" [URL="/heath/V/21/", target="_top"];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "V.3" [URL="/heath/V/3/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "I.38" [URL="/heath/I/38/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "V.16" [URL="/heath/V/16/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "X.11" [URL="/heath/X/11/", target="_top"];
     "X.8" [URL="/heath/X/8/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "V.12" [URL="/heath/V/12/", target="_top"];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "V.2" [style="rounded,filled", fillcolor=orange, URL="/heath/V/2/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "X.5" [URL="/heath/X/5/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "VI.19.p.1" [style="rounded,filled", fillcolor=orange];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "X.7" [URL="/heath/X/7/", target="_top"];
     "V.22" [URL="/heath/V/22/", target="_top"];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "V.15" [URL="/heath/V/15/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "I.31" -> "I.23";
     "I.10" -> "I.9";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "V.16" -> "elem.5.17 elem.5.18";
     "X.5" -> "V.7.p.1";
     "X.6" -> "V.7.p.1";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "V.16" -> "V.14";
     "X.7" -> "X.6";
     "X.11" -> "X.6";
     "X.6" -> "V.9";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "V.22" -> "V.4";
     "X.21" -> "X.def.4";
     "X.21" -> "VI.1";
     "V.16" -> "V.11";
     "VI.1" -> "V.11";
     "X.6" -> "V.11";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "X.5" -> "VII.def.20";
     "X.6" -> "VII.def.20";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "V.8" -> "V.def.4";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "V.16" -> "V.21";
     "I.3" -> "I.2";
     "V.4" -> "V.3";
     "I.15" -> "I.13";
     "I.29" -> "I.13";
     "I.15" -> "I.post.4";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "VI.1" -> "I.38";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "I.16" -> "I.15";
     "I.29" -> "I.15";
     "I.16" -> "I.10";
     "I.13" -> "I.11";
     "I.27" -> "I.def.23";
     "I.34" -> "I.26";
     "VI.1" -> "V.16";
     "I.36" -> "I.33";
     "X.21" -> "X.11";
     "X.11" -> "X.8";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "V.15" -> "V.12";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "V.3" -> "V.2";
     "I.8" -> "I.7";
     "I.38" -> "I.36";
     "X.8" -> "X.5";
     "X.11" -> "X.5";
     "I.7" -> "I.5";
     "I.41" -> "I.37";
     "X.6" -> "VI.19.p.1";
     "I.2" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "X.11" -> "X.7";
     "X.5" -> "V.22";
     "X.6" -> "V.22";
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.29" -> "I.post.5";
     "I.4" -> "I.cn.4";
     "VI.1" -> "I.41";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
   }



Required for
------------

:ref:`X.100`, :ref:`X.101`, :ref:`X.102`, :ref:`X.104`, :ref:`X.105`, :ref:`X.106`, :ref:`X.107`, :ref:`X.108`, :ref:`X.109`, :ref:`X.110`, :ref:`X.111`, :ref:`X.22`, :ref:`X.23`, :ref:`X.25`, :ref:`X.26`, :ref:`X.27`, :ref:`X.28`, :ref:`X.31`, :ref:`X.32`, :ref:`X.33`, :ref:`X.34`, :ref:`X.35`, :ref:`X.38`, :ref:`X.39`, :ref:`X.40`, :ref:`X.41`, :ref:`X.42`, :ref:`X.43`, :ref:`X.44`, :ref:`X.45`, :ref:`X.46`, :ref:`X.47`, :ref:`X.55`, :ref:`X.56`, :ref:`X.57`, :ref:`X.58`, :ref:`X.59`, :ref:`X.60`, :ref:`X.61`, :ref:`X.62`, :ref:`X.63`, :ref:`X.64`, :ref:`X.65`, :ref:`X.67`, :ref:`X.68`, :ref:`X.69`, :ref:`X.70`, :ref:`X.71`, :ref:`X.72`, :ref:`X.75`, :ref:`X.76`, :ref:`X.77`, :ref:`X.78`, :ref:`X.79`, :ref:`X.80`, :ref:`X.81`, :ref:`X.82`, :ref:`X.83`, :ref:`X.84`, :ref:`X.91`, :ref:`X.92`, :ref:`X.93`, :ref:`X.94`, :ref:`X.95`, :ref:`X.96`, :ref:`X.97`, :ref:`X.98`, :ref:`X.99`, :ref:`XIII.11`, :ref:`XIII.16`, :ref:`XIII.17`, :ref:`XIII.18`, :ref:`XIII.6`