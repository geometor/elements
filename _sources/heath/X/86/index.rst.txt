...
Now let it be contrived that, as FD is to DE, so is the square on CG to the square on GB. [:ref:`X.6.p.1`]
...
Since then, as the square on BG is to the square on GC, so is the number ED to the number DF, therefore, convertendo, as the square on BG is to the square on H, so is DE to EF. [:ref:`V.19.p.1`]
...
Therefore BC is a second apotome. [:ref:`X.def.3.2`]
