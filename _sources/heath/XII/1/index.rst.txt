:order: 1
:number: 572
:type: prop
:tags: circle
:dependencies: I.32, III.27, III.31, VI.20, VI.4, VI.6




.. figure:: XII.1.graphic.inverted.png

.. _XII.1:

XII.1
=====

   Similar polygons inscribed in circles are to one another as the squares on the diameters.

Let ABC, FGH be circles, let ABCDE, FGHKL be similar polygons inscribed in them, and let BM, GN be diameters of the circles; I say that, as the square on BM is to the square on GN, so is the polygon ABCDE to the polygon FGHKL.

For let BE, AM, GL, FN be joined.

Now, since the polygon ABCDE is similar to the polygon FGHKL, the angle BAE is equal to the angle GFL, and, as BA is to AE, so is GF to FL. [VI. Def. I]

Thus BAE, GFL are two triangles which have one angle equal to one angle, namely the angle BAE to the angle GFL, and the sides about the equal angles proportional; therefore the triangle ABE is equiangular with the triangle FGL. [:ref:`VI.6`]

Therefore the angle AEB is equal to the angle FLG.

But the angle AEB is equal to the angle AMB, for they stand on the same circumference; [:ref:`III.27`] and the angle FLG to the angle FNG; therefore the angle AMB is also equal to the angle FNG.

But the right angle BAM is also equal to the right angle GFN; [:ref:`III.31`] therefore the remaining angle is equal to the remaining angle. [:ref:`I.32`]

Therefore the triangle ABM is equiangular with the triangle FGN.

Therefore, proportionally, as BM is to GN, so is BA to GF. [:ref:`VI.4`]

But the ratio of the square on BM to the square on GN is duplicate of the ratio of BM to GN, and the ratio of the polygon ABCDE to the polygon FGHKL is duplicate of the ratio of BA to GF; [:ref:`VI.20`] therefore also, as the square on BM is to the square on GN, so is the polygon ABCDE to the polygon FGHKL.

Therefore etc. Q. E. D.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "I.28" [URL="/heath/I/28/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "III.20" [URL="/heath/III/20/", target="_top"];
     "III.31" [URL="/heath/III/31/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "VI.6" [URL="/heath/VI/6/", target="_top"];
     "III.def.11" [style="rounded,filled", fillcolor=orange, URL="/heath/III/def.11/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "VI.1" [URL="/heath/VI/1/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "I.14" [URL="/heath/I/14/", target="_top"];
     "V.16" [URL="/heath/V/16/", target="_top"];
     "VI.11" [URL="/heath/VI/11/", target="_top"];
     "VI.4" [URL="/heath/VI/4/", target="_top"];
     "V.3" [URL="/heath/V/3/", target="_top"];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "I.17" [URL="/heath/I/17/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "VI.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/VI/def.1/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "I.32" [URL="/heath/I/32/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "III.22" [URL="/heath/III/22/", target="_top"];
     "I.38" [URL="/heath/I/38/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "V.21" [URL="/heath/V/21/", target="_top"];
     "III.5" [URL="/heath/III/5/", target="_top"];
     "elem.6.4 elem.6.def.1" [style="rounded,filled", fillcolor=orange];
     "I.39" [URL="/heath/I/39/", target="_top"];
     "V.def.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.11/", target="_top"];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "III.26" [URL="/heath/III/26/", target="_top"];
     "V.14" [URL="/heath/V/14/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "elem.3.1.p.1" [style="rounded,filled", fillcolor=orange];
     "V.12" [URL="/heath/V/12/", target="_top"];
     "V.22" [URL="/heath/V/22/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "VI.2" [URL="/heath/VI/2/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "V.15" [URL="/heath/V/15/", target="_top"];
     "XII.1" [style="rounded,filled", fillcolor=lightblue, URL="/heath/XII/1/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "III.24" [URL="/heath/III/24/", target="_top"];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "V.def.9" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.9/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "V.4" [URL="/heath/V/4/", target="_top"];
     "VI.20" [URL="/heath/VI/20/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "III.10" [URL="/heath/III/10/", target="_top"];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "III.27" [URL="/heath/III/27/", target="_top"];
     "VI.15" [URL="/heath/VI/15/", target="_top"];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "V.2" [style="rounded,filled", fillcolor=orange, URL="/heath/V/2/", target="_top"];
     "III.21" [URL="/heath/III/21/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "VI.19" [URL="/heath/VI/19/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "III.31" -> "I.def.10";
     "I.13" -> "I.11";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "VI.4" -> "I.28";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "III.21" -> "III.20";
     "III.27" -> "III.20";
     "XII.1" -> "III.31";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "V.16" -> "V.11";
     "VI.1" -> "V.11";
     "VI.2" -> "V.11";
     "VI.6" -> "V.11";
     "VI.15" -> "V.11";
     "VI.19" -> "V.11";
     "VI.20" -> "VI.6";
     "XII.1" -> "VI.6";
     "III.26" -> "III.def.11";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "VI.2" -> "VI.1";
     "VI.15" -> "VI.1";
     "VI.19" -> "VI.1";
     "VI.20" -> "VI.1";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "VI.15" -> "I.14";
     "VI.1" -> "V.16";
     "VI.4" -> "V.16";
     "VI.19" -> "V.16";
     "VI.19" -> "VI.11";
     "VI.6" -> "VI.4";
     "XII.1" -> "VI.4";
     "V.4" -> "V.3";
     "I.38" -> "I.36";
     "VI.1" -> "I.41";
     "I.28" -> "I.27";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "I.39" -> "I.37";
     "I.41" -> "I.37";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "VI.4" -> "I.34";
     "I.3" -> "I.2";
     "I.27" -> "I.def.23";
     "III.31" -> "I.17";
     "VI.4" -> "I.17";
     "I.36" -> "I.33";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "VI.20" -> "VI.def.1";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
     "I.10" -> "I.9";
     "III.20" -> "I.32";
     "III.22" -> "I.32";
     "III.31" -> "I.32";
     "VI.6" -> "I.32";
     "VI.20" -> "I.32";
     "XII.1" -> "I.32";
     "V.8" -> "V.def.4";
     "I.4" -> "I.cn.4";
     "I.16" -> "I.15";
     "I.28" -> "I.15";
     "I.29" -> "I.15";
     "III.31" -> "III.22";
     "VI.1" -> "I.38";
     "VI.2" -> "I.38";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "I.7" -> "I.5";
     "III.20" -> "I.5";
     "III.31" -> "I.5";
     "V.16" -> "V.21";
     "III.10" -> "III.5";
     "VI.20" -> "elem.6.4 elem.6.def.1";
     "VI.2" -> "I.39";
     "VI.19" -> "V.def.11";
     "I.34" -> "I.26";
     "III.27" -> "III.26";
     "V.16" -> "V.14";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "III.10" -> "elem.3.1.p.1";
     "V.15" -> "V.12";
     "VI.20" -> "V.12";
     "VI.4" -> "V.22";
     "VI.20" -> "V.22";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "VI.4" -> "VI.2";
     "VI.11" -> "VI.2";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "III.5" -> "I.def.15";
     "III.10" -> "I.def.15";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.39" -> "I.cn.1";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.2" -> "V.7";
     "VI.15" -> "V.7";
     "VI.2" -> "V.9";
     "VI.6" -> "V.9";
     "VI.15" -> "V.9";
     "I.31" -> "I.23";
     "III.27" -> "I.23";
     "VI.6" -> "I.23";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "III.26" -> "III.24";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "VI.19" -> "V.def.9";
     "I.8" -> "I.7";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "VI.11" -> "I.3";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "III.26" -> "I.4";
     "VI.6" -> "I.4";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.28" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "V.22" -> "V.4";
     "XII.1" -> "VI.20";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.39" -> "I.31";
     "VI.11" -> "I.31";
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "I.17" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "III.24" -> "III.10";
     "V.16" -> "elem.5.17 elem.5.18";
     "XII.1" -> "III.27";
     "VI.19" -> "VI.15";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "III.24" -> "I.8";
     "V.3" -> "V.2";
     "III.22" -> "III.21";
     "I.29" -> "I.post.5";
     "VI.4" -> "I.post.5";
     "VI.20" -> "VI.19";
     "I.16" -> "I.10";
   }



Required for
------------

:ref:`XII.10`, :ref:`XII.11`, :ref:`XII.12`, :ref:`XII.13`, :ref:`XII.14`, :ref:`XII.15`, :ref:`XII.2`