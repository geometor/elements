...
:number: 322
:type: prop
:dependencies: V.def.10, VII.17, VII.18




.. figure:: VIII.12.graphic.inverted.png
...
And, since C by multiplying the numbers C, D has made E, F respectively, therefore, as C is to D, so is E to F. [:ref:`VII.17`]
...
.. container:: center
   ...
   as C is to D, so is F to G. [:ref:`VII.18`]


Again, since C by multiplying the numbers E, F has made A, H respectively, therefore, as E is to F, so is A to H. [:ref:`VII.17`]

But, as E is to F, so is C to D.

Therefore also, as C is to D, so is A to H.

Again, since the numbers C, D by multiplying F have made H, K respectively, therefore, as C is to D, so is H to K. [:ref:`VII.18`]

Again, since D by multiplying each of the numbers F, G has made K, B respectively, therefore, as F is to G, so is K to B. [:ref:`VII.17`]
...
For, since A, H, K, B are four numbers in proportion, therefore A has to B the ratio triplicate of that which A has to H. [:ref:`V.def.10`]
...
.. graphviz::
   ...
   digraph {
     ...
     "VIII.12" [style="rounded,filled", fillcolor=lightblue, URL="/heath/VIII/12/", target="_top"];
