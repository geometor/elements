...
:dependencies: I.23, I.32, III.32, elem.3.16.p.1
...
Let ``GH`` be drawn touching the circle ``ABC`` at ``A`` [:ref:`elem.3.16.p.1`]; on the straight line ``AH``, and at the point ``A`` on it, let the angle ``HAC`` be constructed equal to the angle ``DEF``, and on the straight line ``AG``, and at the point ``A`` on it, let the angle ``GAB`` be constructed equal to the angle ``DFE``; [:ref:`I.23`] let ``BC`` be joined.
...
.. graphviz::
   ...
   digraph {
     ...
     "IV.2" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/IV/2/", target="_top"];
