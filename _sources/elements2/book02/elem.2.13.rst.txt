...
For, since the straight line ``CB`` has been cut at random at ``D``, the squares on CB, BD are equal to twice the rectangle contained by CB, BD and the square on DC. [II. 7]
...
But the square on ``AB`` is equal to the squares on ``BD``, ``DA``, for the angle at ``D`` is right; [:ref:`I. 47 <elem.1.47>`] and the square on ``AC`` is equal to the squares on ``AD``, ``DC``; therefore the squares on ``CB``, ``BA`` are equal to the square on ``AC`` and twice the rectangle ``CB``, ``BD``,
