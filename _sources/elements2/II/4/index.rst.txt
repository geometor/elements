...
For let the square ``ADEB`` be described on ``AB``, [:ref:`I.46`]
let ``BD`` be joined; through ``C`` let ``CF`` be drawn parallel to either ``AD`` or ``EB``, and through ``G`` let ``HK`` be drawn parallel to either ``AB`` or ``DE``. [:ref:`I.31`]
