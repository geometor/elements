...
:original_id: I.39
:type: prop
:dependencies: G.44, G.50, G.1
:excerpt: Equal triangles which are on the same base and on the same side are also in the same parallels.

**Heath ID:** `I.39 <https://geometor.github.io/euclid/I/39/>`_
...
For, if not, let ``AE`` be drawn through the point ``A`` parallel to the straight line ``BC``, [:ref:`G.44`] and let ``EC`` be joined. 

Therefore the triangle ``ABC`` is equal to the triangle ``EBC``; for it is on the same base ``BC`` with it and in the same parallels. [:ref:`G.50`]

But ``ABC`` is equal to ``DBC``; therefore ``DBC`` is also equal to ``EBC``, [:ref:`G.1`] the greater to the less: which is impossible.
...
.. graphviz::
   ...
   digraph {
     ...
     "G.52" [label="G.52", penwidth=3, color="white", fillcolor="#555555", URL="/elements2/G.52/", target="_top"];
