...
therefore, ex aequali, as ``AB`` is to ``BG``, so is ``DE`` to ``EH``. [:ref:`V.22`]

And, since the magnitudes are proportional separando, they will also be proportional componendo; [:ref:`V.18`]
...
But also, as ``BG`` is to ``C``, so is ``EH`` to ``F``; therefore, ex aequali, as ``AG`` is to ``C``, so is ``DH`` to ``F``. [:ref:`V.22`]
...
.. graphviz::
   ...
   digraph {
     ...
     "V.24" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/V/24/", target="_top"];
