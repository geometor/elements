...
Let the angle ``ABC`` be greater; and on the straight line ``AB``, and at the point ``B`` on it, let the angle ``ABG`` be constructed equal to the angle ``DEF``. [:ref:`I.23`]

Then, since the angle ``A`` is equal to ``D``, and the angle ``ABG`` to the angle ``DEF``, therefore the remaining angle ``AGB`` is equal to the remaining angle ``DFE``. [:ref:`I.32`]

Therefore the triangle ``ABG`` is equiangular with the triangle ``DEF``.

Therefore, as ``AB`` is to ``BG``, so is ``DE`` to ``EF`` [:ref:`VI.4`]

But, as ``DE`` is to ``EF``, so by hypothesis is ``AB`` to ``BC``; therefore ``AB`` has the same ratio to each of the straight lines ``BC``, ``BG``; [:ref:`V.11`]


.. container:: center

   therefore ``BC`` is equal to ``BG``, [:ref:`V.9`]


so that the angle at ``C`` is also equal to the angle ``BGC``. [:ref:`I.5`]

But, by hypothesis, the angle at ``C`` is less than a right angle; therefore the angle ``BGC`` is also less than a right angle; so that the angle ``AGB`` adjacent to it is greater than a right angle. [:ref:`I.13`]
...
But the angle at ``A`` is also equal to the angle at ``D``; therefore the remaining angle at ``C`` is equal to the remaining angle at ``F``. [:ref:`I.32`]
...
Thus in the triangle ``BGC`` the two angles are not less than two right angles: which is impossible. [:ref:`I.17`]
...
.. graphviz::
   ...
   digraph {
     ...
     "VI.7" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/VI/7/", target="_top"];
