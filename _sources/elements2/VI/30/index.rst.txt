...
On ``AB`` let the square ``BC`` be described; and let there be applied to ``AC`` the parallelogram ``CD`` equal to ``BC`` and exceeding by the figure ``AD`` similar to ``BC``. [:ref:`VI.29`]
...
But it is also equiangular with it; therefore in ``BF``, ``AD`` the sides about the equal angles are reciprocally proportional; [:ref:`VI.14`]
...
.. graphviz::
   ...
   digraph {
     ...
     "VI.14" [URL="/elements2/VI/14/", target="_top"];
     ...
     "VI.30" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/VI/30/", target="_top"];
     ...
     "VI.29" [URL="/elements2/VI/29/", target="_top"];
