...
and, as ``DC`` is to ``CE``, so is ``L`` to ``M``. [:ref:`VI.12`]
...
Now since, as ``BC`` is to ``CG``, so is the parallelogram ``AC`` to the parallelogram ``CH``, [:ref:`VI.1`] while, as ``BC`` is to ``CG``, so is ``K`` to ``L``, therefore also, as ``K`` is to ``L``, so is ``AC`` to ``CH``. [:ref:`V.11`]

Again, since, as ``DC`` is to ``CE``, so is the parallelogram ``CH`` to ``CF``, [:ref:`VI.1`] while, as ``DC`` is to ``CE``, so is ``L`` to ``M``, therefore also, as ``L`` is to ``M``, so is the parallelogram ``CH`` to the parallelogram ``CF``. [:ref:`V.11`]
...
.. graphviz::
   ...
   digraph {
     ...
     "VI.23" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/VI/23/", target="_top"];
