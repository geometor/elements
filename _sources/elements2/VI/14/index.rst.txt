:order: 14
:number: 230
:type: prop
:dependencies: I.14, V.11, V.7, V.9, VI.1




.. figure:: VI.14.jpg
   :width: 50%

.. _VI.14:

VI.14
=====

   In equal and equiangular parallelograms the sides about the equal angles are reciprocally proportional; and equiangular parallelograms in which the sides about the equal angles are reciprocally proportional are equal.

Let ``AB``, ``BC`` be equal and equiangular parallelograms having the angles at ``B`` equal, and let ``DB``, ``BE`` be placed in a straight line;


.. container:: center

   therefore ``FB``, ``BG`` are also in a straight line. [:ref:`I.14`]


I say that, in ``AB``, ``BC``, the sides about the equal angles are reciprocally proportional, that is to say, that, as ``DB`` is to ``BE``, so is ``GB`` to ``BF``.

For let the parallelogram ``FE`` be completed.

Since, then, the parallelogram ``AB`` is equal to the parallelogram ``BC``,


.. container:: center

   and ``FE`` is another area,


therefore, as ``AB`` is to ``FE``, so is ``BC`` to ``FE``. [:ref:`V.7`]

But, as ``AB`` is to ``FE``, so is ``DB`` to ``BE``, [:ref:`VI.1`] and, as ``BC`` is to ``FE``, so is ``GB`` to ``BF``. [``id``.] therefore also, as ``DB`` is to ``BE``, so is ``GB`` to ``BF``. [:ref:`V.11`]

Therefore in the parallelograms ``AB``, ``BC`` the sides about the equal angles are reciprocally proportional.

Next, let ``GB`` be to ``BF`` as ``DB`` to ``BE``; I say that the parallelogram ``AB`` is equal to the parallelogram ``BC``.

For since, as ``DB`` is to ``BE``, so is ``GB`` to ``BF``, while, as ``DB`` is to ``BE``, so is the parallelogram ``AB`` to the parallelogram ``FE``, [:ref:`VI.1`] and, as ``GB`` is to ``BF``, so is the parallelogram ``BC`` to the parallelogram ``FE``, [:ref:`VI.1`] therefore also, as ``AB`` is to ``FE``, so is ``BC`` to ``FE``; [:ref:`V.11`] therefore the parallelogram ``AB`` is equal to the parallelogram ``BC``. [:ref:`V.9`]

Therefore etc. Q. E. D.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "V.20" [URL="/elements2/V/20/", target="_top"];
     "I.8" [URL="/elements2/I/8/", target="_top"];
     "I.36" [URL="/elements2/I/36/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/elements2/V/def.5/", target="_top"];
     "VI.14" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/VI/14/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/elements2/V/def.4/", target="_top"];
     "I.14" [URL="/elements2/I/14/", target="_top"];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/post.1/", target="_top"];
     "I.33" [URL="/elements2/I/33/", target="_top"];
     "I.5" [URL="/elements2/I/5/", target="_top"];
     "I.38" [URL="/elements2/I/38/", target="_top"];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "VI.1" [URL="/elements2/VI/1/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/elements2/V/11/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/def.10/", target="_top"];
     "V.13" [URL="/elements2/V/13/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/post.5/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/cn.2/", target="_top"];
     "V.9" [URL="/elements2/V/9/", target="_top"];
     "V.16" [URL="/elements2/V/16/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/def.15/", target="_top"];
     "V.15" [URL="/elements2/V/15/", target="_top"];
     "I.31" [URL="/elements2/I/31/", target="_top"];
     "I.11" [URL="/elements2/I/11/", target="_top"];
     "I.3" [URL="/elements2/I/3/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/def.23/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/post.2/", target="_top"];
     "I.26" [URL="/elements2/I/26/", target="_top"];
     "V.14" [URL="/elements2/V/14/", target="_top"];
     "V.10" [URL="/elements2/V/10/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/cn.3/", target="_top"];
     "I.1" [URL="/elements2/I/1/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/cn.4/", target="_top"];
     "I.23" [URL="/elements2/I/23/", target="_top"];
     "I.7" [URL="/elements2/I/7/", target="_top"];
     "I.9" [URL="/elements2/I/9/", target="_top"];
     "I.16" [URL="/elements2/I/16/", target="_top"];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/elements2/V/1/", target="_top"];
     "V.7" [URL="/elements2/V/7/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/cn.1/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/post.4/", target="_top"];
     "V.12" [URL="/elements2/V/12/", target="_top"];
     "I.13" [URL="/elements2/I/13/", target="_top"];
     "V.21" [URL="/elements2/V/21/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "I.41" [URL="/elements2/I/41/", target="_top"];
     "I.29" [URL="/elements2/I/29/", target="_top"];
     "I.35" [URL="/elements2/I/35/", target="_top"];
     "I.2" [URL="/elements2/I/2/", target="_top"];
     "I.37" [URL="/elements2/I/37/", target="_top"];
     "I.27" [URL="/elements2/I/27/", target="_top"];
     "I.4" [URL="/elements2/I/4/", target="_top"];
     "V.8" [URL="/elements2/V/8/", target="_top"];
     "I.10" [URL="/elements2/I/10/", target="_top"];
     "I.15" [URL="/elements2/I/15/", target="_top"];
     "I.34" [URL="/elements2/I/34/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/elements2/V/def.7/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/elements2/I/post.3/", target="_top"];
     "V.16" -> "V.20";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "I.38" -> "I.36";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "V.8" -> "V.def.4";
     "VI.14" -> "I.14";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "I.36" -> "I.33";
     "I.7" -> "I.5";
     "VI.1" -> "I.38";
     "V.16" -> "elem.5.17 elem.5.18";
     "VI.14" -> "VI.1";
     "V.16" -> "V.11";
     "VI.1" -> "V.11";
     "VI.14" -> "V.11";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "I.29" -> "I.post.5";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "VI.14" -> "V.9";
     "VI.1" -> "V.16";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.13" -> "I.11";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "I.27" -> "I.def.23";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.34" -> "I.26";
     "V.16" -> "V.14";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "I.4" -> "I.cn.4";
     "I.31" -> "I.23";
     "I.8" -> "I.7";
     "I.10" -> "I.9";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.14" -> "V.7";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "V.15" -> "V.12";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.29" -> "I.13";
     "V.16" -> "V.21";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
     "VI.1" -> "I.41";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "I.3" -> "I.2";
     "I.41" -> "I.37";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "I.16" -> "I.10";
     "I.16" -> "I.15";
     "I.29" -> "I.15";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
   }



Required for
------------

:ref:`VI.16`, :ref:`VI.17`, :ref:`VI.30`