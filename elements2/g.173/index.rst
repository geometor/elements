...
:original_id: VI.7
:type: prop
:dependencies: g.21, g.26, g.33, g.45, g.12, g.150, g.148, g.170
...
Let the angle ``ABC`` be greater; and on the straight line ``AB``, and at the point ``B`` on it, let the angle ``ABG`` be constructed equal to the angle ``DEF``. [:ref:`g.33`]

Then, since the angle ``A`` is equal to ``D``, and the angle ``ABG`` to the angle ``DEF``, therefore the remaining angle ``AGB`` is equal to the remaining angle ``DFE``. [:ref:`g.45`]

Therefore the triangle ``ABG`` is equiangular with the triangle ``DEF``.

Therefore, as ``AB`` is to ``BG``, so is ``DE`` to ``EF`` [:ref:`g.170`]

But, as ``DE`` is to ``EF``, so by hypothesis is ``AB`` to ``BC``; therefore ``AB`` has the same ratio to each of the straight lines ``BC``, ``BG``; [:ref:`g.150`]


.. container:: center

   therefore ``BC`` is equal to ``BG``, [:ref:`g.148`]


so that the angle at ``C`` is also equal to the angle ``BGC``. [:ref:`g.12`]

But, by hypothesis, the angle at ``C`` is less than a right angle; therefore the angle ``BGC`` is also less than a right angle; so that the angle ``AGB`` adjacent to it is greater than a right angle. [:ref:`g.21`]
...
But the angle at ``A`` is also equal to the angle at ``D``; therefore the remaining angle at ``C`` is equal to the remaining angle at ``F``. [:ref:`g.45`]
...
Thus in the triangle ``BGC`` the two angles are not less than two right angles: which is impossible. [:ref:`g.26`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.173" [label="G.173", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.173/", target="_top"];
