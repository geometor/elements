...
On ``AB`` let the square ``BC`` be described; and let there be applied to ``AC`` the parallelogram ``CD`` equal to ``BC`` and exceeding by the figure ``AD`` similar to ``BC``. [:ref:`g.201`]
...
But it is also equiangular with it; therefore in ``BF``, ``AD`` the sides about the equal angles are reciprocally proportional; [:ref:`g.182`]
