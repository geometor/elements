...
For, since the straight line ``CD`` has been cut at random at the point ``A``, the square on ``DC`` is equal to the squares on ``CA``, ``AD`` and twice the rectangle contained by ``CA``, ``AD``. [:ref:`g.65`]
...
But the square on ``CB`` is equal to the squares on ``CD``, ``DB``, for the angle at ``D`` is right; [:ref:`g.60`]


.. container:: center

   and the square on ``AB`` is equal to the squares on ``AD``, ``DB``; [:ref:`g.60`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.73" [label="G.73", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.73/", target="_top"];
