...
Let the angle ``BGC`` be greater : and on the straight line ``BG``, and at the point ``G`` on it, let the angle ``BGK`` be constructed equal to the angle ``EHF``. [:ref:`g.33`]

Now equal angles stand on equal circumferences, when they are at the centres; [:ref:`g.107`]
...
.. container:: center
   ...
   and the angle at ``D`` half of the angle ``EHF``; [:ref:`g.100`]
