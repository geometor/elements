...
:original_id: X.11
:type: prop
:dependencies: g.337, g.338, g.339, g.340
...
For, since A is commensurable with B, therefore A has to B the ratio which a number has to a number. [:ref:`g.337`]

And, as A is to B, so is C to D; therefore C also has to D the ratio which a number has to a number; therefore C is commensurable with D. [:ref:`g.338`]

Next, let A be incommensurable with B; I say that C will also be incommensurable with D.

For, since A is incommensurable with B, therefore A has not to B the ratio which a number has to a number. [:ref:`g.339`]

And, as A is to B, so is C to D; therefore neither has C to D the ratio which a number has to a number; therefore C is incommensurable with D. [:ref:`g.340`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.343" [label="G.343", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.343/", target="_top"];
