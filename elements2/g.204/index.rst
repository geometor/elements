...
:original_id: VI.32
:type: prop
:dependencies: g.23, g.42, g.45, g.172
...
For, since ``AB`` is parallel to ``DC``, and the straight line ``AC`` has fallen upon them, the alternate angles ``BAC``, ``ACD`` are equal to one another. [:ref:`g.42`]
...
.. container:: center
   ...
   therefore the triangle ``ABC`` is equiangular with the triangle ``DCE``; [:ref:`g.172`] therefore the angle ``ABC`` is equal to the angle ``DCE``.
...
But the angles ``BAC``, ``ABC``, ``ACB`` are equal to two right angles; [:ref:`g.45`]
...
.. container:: center
   ...
   therefore ``BC`` is in a straight line with ``CE``. [:ref:`g.23`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.204" [label="G.204", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.204/", target="_top"];
