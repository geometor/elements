...
:original_id: X.52
:type: prop
:dependencies: g.381, g.342, g.344, g.370
...
Let it be contrived that, as CA is to AB, so is the square on EF to the square on FG. [:ref:`g.344`]
...
Therefore EF, FG are rational straight lines commensurable in square only; [:ref:`g.342`] therefore EG is binomial. [:ref:`g.381`]
...
Let then the squares on EF, H be equal to the square on GF; therefore, convertendo, as the number AB is to BC, so is the square on GF to the square on H. [:ref:`g.370`]
...
Therefore FG is incommensurable in length with H; [:ref:`g.342`] so that the square on FG is greater than the square on FE by the square on a straight line incommensurable with FG.
...
.. graphviz::
   ...
   digraph {
     ...
     "G.399" [label="G.399", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.399/", target="_top"];
