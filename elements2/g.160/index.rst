...
Then, since ``GH`` is the same multiple of ``AE`` that ``HK`` is of ``EB``, therefore ``GH`` is the same multiple of ``AE`` that ``GK`` is of ``AB``. [:ref:`g.137`]

But ``GH`` is the same multiple of ``AE`` that ``LM`` is of ``CF``; therefore ``GK`` is the same multiple of ``AB`` that ``LM`` is of ``CF``.

Again, since ``LM`` is the same multiple of ``CF`` that ``MN`` is of ``FD``, therefore ``LM`` is the same multiple of ``CF`` that ``LN`` is of ``CD``. [:ref:`g.137`]
...
.. container:: center
   ...
   and ``KO`` is also the same multiple of ``EB`` that ``NP`` is of ``FD``, therefore the sum ``HO`` is also the same multiple of ``EB`` that ``MP`` is of ``FD``. [:ref:`g.138`]
