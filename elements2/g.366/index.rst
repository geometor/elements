...
:original_id: X.27
:type: prop
:dependencies: g.159, g.179, g.181, g.185, g.343, g.357, g.361
...
Let two rational straight lines A, B commensurable in square only be set out; let C be taken a mean proportional between A, B, [:ref:`g.181`] and let it be contrived that,


.. container:: center

   as A is to B, so is C to D. [:ref:`g.179`]


Then, since A, B are rational and commensurable in square only, the rectangle A, B, that is, the square on C [:ref:`g.185`], is medial. [:ref:`g.357`]

Therefore C is medial. [:ref:`g.357`]

And since, as A is to B, so is C to D, and A, B are commensurable in square only, therefore C, D are also commensurable in square only. [:ref:`g.343`]

And C is medial; therefore D is also medial. [:ref:`g.361`, addition]

Therefore C, D are medial and commensurable in square only.

I say that they also contain a rational rectangle.

For since, as A is to B, so is C to D, therefore, alternately, as A is to C, so is B to D. [:ref:`g.159`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.366" [label="G.366", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.366/", target="_top"];
