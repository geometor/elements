...
For let them be produced to the points ``D``, ``E``, and let ``BD`` be made equal to ``AC``; [:ref:`g.9`] let ``BC`` be joined, and through ``D`` let ``DE`` be drawn parallel to it. [:ref:`g.44`]

Since, then, ``BC`` has been drawn parallel to ``DE``, one of the sides of the triangle ``ADE``, proportionally, as ``AB`` is to ``BD``, so is ``AC`` to ``CE``. [:ref:`g.168`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.178" [label="G.178", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.178/", target="_top"];
