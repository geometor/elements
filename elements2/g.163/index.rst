...
:original_id: V.22
:type: prop
:dependencies: g.155, g.141, g.140
...
.. container:: center
   ...
   therefore, as ``G`` is to ``K``, so is ``H`` to ``L``. [:ref:`g.141`]
...
Since, then, there are three magnitudes ``G``, ``K``, ``M``, and others ``H``, ``L``, ``N`` equal to them in multitude, which taken two and two together are in the same ratio, therefore, ex aequali, if ``G`` is in excess of ``M``, ``H`` is also in excess of ``N``; if equal, equal; and if less, less. [:ref:`g.155`]
...
Therefore, as ``A`` is to ``C``, so is ``D`` to ``F``. [:ref:`g.140`]
