...
:original_id: VI.23
:type: prop
:dependencies: g.150, g.167, g.179
...
and, as ``DC`` is to ``CE``, so is ``L`` to ``M``. [:ref:`g.179`]
...
Now since, as ``BC`` is to ``CG``, so is the parallelogram ``AC`` to the parallelogram ``CH``, [:ref:`g.167`] while, as ``BC`` is to ``CG``, so is ``K`` to ``L``, therefore also, as ``K`` is to ``L``, so is ``AC`` to ``CH``. [:ref:`g.150`]

Again, since, as ``DC`` is to ``CE``, so is the parallelogram ``CH`` to ``CF``, [:ref:`g.167`] while, as ``DC`` is to ``CE``, so is ``L`` to ``M``, therefore also, as ``L`` is to ``M``, so is the parallelogram ``CH`` to the parallelogram ``CF``. [:ref:`g.150`]
