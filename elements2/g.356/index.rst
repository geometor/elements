...
:original_id: XI.24
:type: prop
:dependencies: g.47, g.11, g.340, g.347
...
For, since the two parallel planes BG, CE are cut by the plane AC, their common sections are parallel. [:ref:`g.347`]

Therefore AB is parallel to DC.

Again, since the two parallel planes BF, AE are cut by the plane AC, their common sections are parallel. [:ref:`g.347`]
...
Then, since AB is parallel to DC, and BH to CF, the two straight lines AB, BH which meet one another are parallel to the two straight lines DC, CF which meet one another, not in the same plane; therefore they will contain equal angles; [:ref:`g.340`] therefore the angle ABH is equal to the angle DCF.
