...
:original_id: IX.10
:type: prop
:dependencies: g.289, g.292, g.281, g.282
...
But B is also square; [:ref:`g.292`] [therefore B, C have to one another the ratio which a square number has to a square number].

And, as B is to C, so is A to B; therefore A, B have to one another the ratio which a square number has to a square number; [so that A, B are similar plane numbers]. [:ref:`g.282`, converse]
...
Now C is also cube; for it is fourth from the unit. [:ref:`g.292`]

And, as C is to D, so is B to C; therefore B also has to C the ratio which a cube has to a cube.

And C is cube; therefore B is also cube. [:ref:`g.281`]
...
But, if a number by multiplying itself make a cube number, it is also itself cube. [:ref:`g.289`]
