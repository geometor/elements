...
:original_id: X.5
:type: prop
:dependencies: g.163, g.219, g.336
...
Since then C measures A according to the units in D, while the unit also measures D according to the units in it, therefore the unit measures the number D the same number of times as the magnitude C measures A; therefore, as C is to A, so is the unit to D; [:ref:`g.219`] therefore, inversely, as A is to C, so is D to the unit. [cf. :ref:`g.336`]
...
.. container:: center
   ...
   as A is to B, so is the number D to E. [:ref:`g.163`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.337" [label="G.337", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.337/", target="_top"];
