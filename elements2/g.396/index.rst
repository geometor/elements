...
:original_id: X.49
:type: prop
:dependencies: g.381, g.338, g.342, g.344, g.370
...
Let it be contrived then that, as the number CA is to AB, so also is the square on EF to the square on FG; [:ref:`g.344`] therefore the square on EF is commensurable with the square on FG. [:ref:`g.338`]
...
Therefore EF is incommensurable in length with FG; [:ref:`g.342`] therefore EF, FG are rational straight lines commensurable in square only; therefore EG is binomial. [:ref:`g.381`]
...
Let the squares on EF, H be equal to the square on GF; therefore, convertendo, as AB is to BC, so is the square on FG to the square on H. [:ref:`g.370`]
...
Therefore FG is commensurable in length with H; [:ref:`g.342`] so that the square on FG is greater than the square on FE by the square on a straight line commensurable with FG.
...
.. graphviz::
   ...
   digraph {
     ...
     "G.396" [label="G.396", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.396/", target="_top"];
