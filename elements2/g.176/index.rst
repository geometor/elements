...
Let a straight line ``AC`` be drawn through from ``A`` containing with ``AB`` any angle; let a point ``D`` be taken at random on ``AC``, and let ``DE``, ``EC`` be made equal to ``AD``. [:ref:`g.9`]

Let ``BC`` be joined, and through ``D`` let ``DF`` be drawn parallel to it. [:ref:`g.44`]

Then, since ``FD`` has been drawn parallel to ``BC``, one of the sides of the triangle ``ABC``, therefore, proportionally, as ``CD`` is to ``DA``, so is ``BF`` to ``FA``. [:ref:`g.168`]
