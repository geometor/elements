...
For let the square ``CDEB`` be described on ``CB``; [:ref:`g.59`] let ``ED`` be drawn through to ``F``, and through ``A`` let ``AF`` be drawn parallel to either ``CD`` or ``BE``. [:ref:`g.44`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.64" [label="G.64", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.64/", target="_top"];
