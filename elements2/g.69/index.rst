...
And, since ``BC`` is equal to ``BD``, and ``GK`` to ``KN``, therefore ``CK`` is also equal to ``KD``, and ``GR`` to ``RN``. [:ref:`g.49`]

But ``CK`` is equal to ``RN``, for they are complements of the parallelogram ``CP``; [:ref:`g.56`] therefore ``KD`` is also equal to ``GR``; therefore the four areas ``DK``, ``CK``, ``GR``, ``RN`` are equal to one another.
...
.. container:: center
   ...
   ``AG`` is also equal to ``MQ``, and ``QL`` to ``RF``. [:ref:`g.49`]


But ``MQ`` is equal to ``QL``, for they are complements of the parallelogram ``ML``; [:ref:`g.56`]
