...
For, since the parallelepipedal solid KA is similar to LC, therefore KA has to LC the ratio triplicate of that which AB has to CD. [:ref:`g.522`]
...
For since, again, KA has to LC the ratio triplicate of that which AB has to CD, [:ref:`g.522`] and ME also has to NG the ratio triplicate of that which EF has to GH, [id.] and, as KA is to LC, so is ME to NG, therefore also, as AB is to CD, so is EF to GH.
...
.. graphviz::
   ...
   digraph {
     ...
     "G.527" [label="G.527", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.527/", target="_top"];
