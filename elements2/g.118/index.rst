...
:original_id: III.37
:type: prop
:dependencies: g.15, g.98, g.117, g.96
...
Thus the angle ``FED`` is right. [:ref:`g.98`]

Now, since ``DE`` touches the circle ``ABC``, and ``DCA`` cuts it, the rectangle ``AD``, ``DC`` is equal to the square on ``DE``. [:ref:`g.117`]
...
.. container:: center
   ...
   therefore the angle ``DEF`` is equal to the angle ``DBF``. [:ref:`g.15`]
...
And ``FB`` produced is a diameter; and the straight line drawn at right angles to the diameter of a circle, from its extremity, touches the circle; [:ref:`g.96`]
