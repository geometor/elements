...
:original_id: X.64
:type: prop
:dependencies: g.347, g.353, g.356, g.358, g.381, g.386
...
Since then AB is the side of a rational plus a medial area, divided at C, therefore AC, CB are straight lines incommensurable in square which make the sum of the squares on them medial, but the rectangle contained by them rational. [:ref:`g.386`]

Since then the sum of the squares on AC, CB is medial, therefore DL is medial, so that DM is rational and incommensurable in length with DE. [:ref:`g.358`]

Again, since twice the rectangle AC, CB, that is MF, is rational, therefore MG is rational and commensurable with DE. [:ref:`g.356`]

Therefore DM is incommensurable with MG; [:ref:`g.347`] therefore DM, MG are rational straight lines commensurable in square only; therefore DG is binomial. [:ref:`g.381`]

I say next that it is also a fifth binomial straight line.

For it can be proved similarly that the rectangle DK, KM is equal to the square on MN, and that DK is incommensurable in length with KM; therefore the square on DM is greater than the square on MG by the square on a straight line incommensurable with DM. [:ref:`g.353`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.420" [label="G.420", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.420/", target="_top"];
