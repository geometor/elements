...
:original_id: III.8
:type: prop
:dependencies: g.30, g.31, g.34, g.11, g.76
...
For let the centre of the circle ``ABC`` be taken [:ref:`g.76`], and let it be ``M``; let ``ME``, ``MF``, ``MC``, ``MK``, ``ML``, ``MH`` be joined.
...
But ``EM``, ``MD`` are greater than ``ED``; [:ref:`g.30`]
...
.. container:: center
   ...
   therefore the base ``ED`` is greater than the base ``FD``. [:ref:`g.34`]


Similarly we can prove that ``FD`` is greater than ``CD``; therefore ``DA`` is greatest, while ``DE`` is greater than ``DF``, and ``DF`` than ``DC``.

Next, since ``MK``, ``KD`` are greater than ``MD``, [:ref:`g.30`] and ``MG`` is equal to ``MK``, therefore the remainder ``KD`` is greater than the remainder ``GD``,
...
And, since on ``MD``, one of the sides of the triangle ``MLD``, two straight lines ``MK``, ``KD`` were constructed meeting within the triangle, therefore ``MK``, ``KD`` are less than ``ML``, ``LD``; [:ref:`g.31`] and ``MK`` is equal to ``ML``;
...
.. container:: center
   ...
   therefore the base ``DK`` is equal to the base ``DB``. [:ref:`g.11`]
