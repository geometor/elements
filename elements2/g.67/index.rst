...
:original_id: II.6
:type: prop
:dependencies: g.44, g.49, g.56, g.59
...
For let the square ``CEFD`` be described on ``CD``, [:ref:`g.59`] and let ``DE`` be joined; through the point ``B`` let ``BG`` be drawn parallel to either ``EC`` or ``DF``, through the point ``H`` let ``KM`` be drawn parallel to either ``AB`` or ``EF``, and further through ``A`` let ``AK`` be drawn parallel to either ``CL`` or ``DM``. [:ref:`g.44`]


.. container:: center

   Then, since ``AC`` is equal to ``CB``, ``AL`` is also equal to ``CH``. [:ref:`g.49`]


But ``CH`` is equal to ``HF``. [:ref:`g.56`]
...
.. graphviz::
   ...
   digraph {
     ...
     "G.67" [label="G.67", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.67/", target="_top"];
...
:ref:`g.72`, :ref:`g.117`, :ref:`g.118`, :ref:`g.131`, :ref:`g.132`, :ref:`g.133`, :ref:`g.477`, :ref:`g.369`, :ref:`g.434`, :ref:`g.441`, :ref:`g.459`, :ref:`g.465`
