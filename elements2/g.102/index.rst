...
Then, since in any triangle the three angles are equal to two right angles, [:ref:`g.45`]
...
But the angle ``CAB`` is equal to the angle ``BDC``, for they are in the same segment ``BADC``; [:ref:`g.101`] and the angle ``ACB`` is equal to the angle
...
.. graphviz::
   ...
   digraph {
     ...
     "G.102" [label="G.102", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.102/", target="_top"];
