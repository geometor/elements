...
.. container:: center
   ...
   the squares on ``CB``, ``BD`` are equal to twice the rectangle contained by ``CB``, ``BD`` and the square on ``DC``. [:ref:`g.68`]
...
But the square on ``AB`` is equal to the squares on ``BD``, ``DA``, for the angle at ``D`` is right; [:ref:`g.60`] and the square on ``AC`` is equal to the squares on ``AD``, ``DC``; therefore the squares on ``CB``, ``BA`` are equal to the square on ``AC`` and twice the rectangle ``CB``, ``BD``,
...
.. graphviz::
   ...
   digraph {
     ...
     "G.74" [label="G.74", style="rounded,filled", fillcolor=lightblue, URL="/elements2/g.74/", target="_top"];
