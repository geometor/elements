...
.. container:: center
   ...
   the angle ``DAC`` is also equal to the angle ``ACD``. [:ref:`g.12`]
...
thus, in the triangle ``ACD``, the two angles ``DAC``, ``ACD`` are equal to two right angles: which is impossible. [:ref:`g.26`]
...
.. container:: center
   ...
   and the angle ``DAG`` is less than a right angle, ``AD`` is greater than ``DG``. [:ref:`g.28`]
