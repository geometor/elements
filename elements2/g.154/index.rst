...
And, since ``AG``. ``GH``, ``HB`` are equal to one another, and ``DK``, ``KL``, ``LE`` are also equal to one another, therefore, as ``AG`` is to ``DK``, so is ``GH`` to ``KL``, and ``HB`` to ``LE``. [:ref:`g.144`]

Therefore, as one of the antecedents is to one of the consequents, so will all the antecedents be to all the consequents; [:ref:`g.151`]
