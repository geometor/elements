...
.. figure:: VI.12.graphic.inverted.png
...
Since, then, ``GH`` has been drawn parallel to ``EF``, one of the sides of the triangle ``DEF``, therefore, as ``DG`` is to ``GE``, so is ``DH`` to ``HF``. [:ref:`VI.2`]
...
.. graphviz::
   ...
   digraph {
     ...
     "VI.12" [style="rounded,filled", fillcolor=lightblue, URL="/heath/VI/12/", target="_top"];
