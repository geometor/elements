...
:categories: describe
:tags: line
:dependencies: V.11, V.22, V.9, VI.11, VI.12, VI.18, VI.19.p.1




.. figure:: VI.22.graphic.inverted.png
...
For let there be taken a third proportional ``O`` to ``AB``, ``CD``, and a third proportional ``P`` to ``EF``, ``GH``. [:ref:`VI.11 <VI.11>`]
...
therefore, ``ex aequali``, as ``AB`` is to ``O``, so is ``EF`` to ``P``. [:ref:`V.22 <V.22>`]

But, as ``AB`` is to ``O``, so is ``KAB`` to ``LCD``, [:ref:`VI.19.p.1 <VI.19.p.1>`]
...
therefore also, as ``KAB`` is to ``LCD``, so is ``MF`` to ``NH``. [:ref:`V.11 <V.11>`]
...
.. container:: center
   ...
   let ``EF`` be to ``QR`` as ``AB`` to ``CD``, [:ref:`VI.12 <VI.12>`]


and on ``QR`` let the rectilineal figure ``SR`` be described similar and similarly situated to either of the two ``MF``, ``NH``. [:ref:`VI.18 <VI.18>`]
...
.. container:: center
   ...
   as ``KAB`` is to ``LCD``, so is ``MF`` to ``NH``; therefore also, as ``MF`` is to ``SR``, so is ``MF`` to ``NH``. [:ref:`V.11 <V.11>`]
...
.. container:: center
   ...
   therefore ``NH`` is equal to ``SR``. [:ref:`V.9 <V.9>`]
...
.. graphviz::
   ...
   digraph {
     ...
     "VI.22" [penwidth=3, color="white", fillcolor="#555555", URL="/heath/VI/22/", target="_top"];
