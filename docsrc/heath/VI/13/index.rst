...
And, since, in the right-angled triangle ``ADC``, ``DB`` has been drawn from the right angle perpendicular to the base, therefore ``DB`` is a mean proportional between the segments of the base, ``AB``, ``BC``. [:ref:`VI.8.p.1`]
