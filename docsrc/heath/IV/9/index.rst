...
.. figure:: IV.9.graphic.inverted.png
...
.. container:: center
   ...
   therefore the angle ``DAC`` is equal to the angle ``BAC``. [:ref:`I.8`]
...
Now, since the angle ``DAB`` is equal to the angle ``ABC``, and the angle ``EAB`` is half the angle ``DAB``, and the angle ``EBA`` half the angle ``ABC``, therefore the angle ``EAB`` is also equal to the angle ``EBA``; so that the side ``EA`` is also equal to ``EB``. [:ref:`I.6`]
...
.. graphviz::
   ...
   digraph {
     ...
     "IV.9" [style="rounded,filled", fillcolor=lightblue, URL="/heath/IV/9/", target="_top"];
