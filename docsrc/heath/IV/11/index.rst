:order: 11
:number: 163
:type: prop
:tags: circle
:dependencies: I.9, III.26, III.27, III.29, IV.10, IV.2




.. figure:: IV.11.graphic.inverted.png

.. _IV.11:

IV.11
=====

   In a given circle to inscribe an equilateral and equiangular pentagon.

Let ``ABCDE`` be the given circle; thus it is required to inscribe in the circle ``ABCDE`` an equilateral and equiangular pentagon.

Let the isosceles triangle ``FGH`` be set out having each of the angles at ``G``, ``H`` double of the angle at ``F``; [:ref:`IV.10`]

let there be inscribed in the circle ``ABCDE`` the triangle ``ACD`` equiangular with the triangle ``FGH``, so that the angle ``CAD`` is equal to the angle at ``F`` and the angles at ``G``, ``H`` respectively equal to the angles ``ACD``, ``CDA``; [:ref:`IV.2`] therefore each of the angles ``ACD``, ``CDA`` is also double of the angle ``CAD``.

Now let the angles ``ACD``, ``CDA`` be bisected respectively by the straight lines ``CE``, ``DB`` [:ref:`I.9`], and let ``AB``, ``BC``, ``DE``, ``EA`` be joined.

Then, since each of the angles ``ACD``, ``CDA`` is double of the angle ``CAD``, and they have been bisected by the straight lines ``CE``, ``DB``, therefore the five angles ``DAC``, ``ACE``, ``ECD``, ``CDB``, ``BDA`` are equal to one another.

But equal angles stand on equal circumferences; [:ref:`III.26`] therefore the five circumferences ``AB``, ``BC``, ``CD``, ``DE``, ``EA`` are equal to one another.

But equal circumferences are subtended by equal straight lines; [:ref:`III.29`] therefore the five straight lines ``AB``, ``BC``, ``CD``, ``DE``, ``EA`` are equal to one another;


.. container:: center

   therefore the pentagon ``ABCDE`` is equilateral.


I say next that it is also equiangular.

For, since the circumference ``AB`` is equal to the circumference ``DE``, let ``BCD`` be added to each;


.. container:: center

   therefore the whole circumference ``ABCD`` is equal to the whole circumference ``EDCB``.


And the angle ``AED`` stands on the circumference ``ABCD``, and the angle ``BAE`` on the circumference ``EDCB``;


.. container:: center

   therefore the angle ``BAE`` is also equal to the angle ``AED``. [:ref:`III.27`]


For the same reason each of the angles ``ABC``, ``BCD``, ``CDE`` is also equal to each of the angles ``BAE``, ``AED``;


.. container:: center

   therefore the pentagon ``ABCDE`` is equiangular.


But it was also proved equilateral;


.. container:: center

   therefore in the given circle an equilateral and equiangular pentagon has been inscribed. Q. E. F.



Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "IV.5" [URL="/heath/IV/5/", target="_top"];
     "III.22" [URL="/heath/III/22/", target="_top"];
     "II.11" [URL="/heath/II/11/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "IV.10" [URL="/heath/IV/10/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "III.10" [URL="/heath/III/10/", target="_top"];
     "III.20" [URL="/heath/III/20/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "III.27" [URL="/heath/III/27/", target="_top"];
     "III.18" [URL="/heath/III/18/", target="_top"];
     "III.19" [URL="/heath/III/19/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "elem.3.16.p.1" [style="rounded,filled", fillcolor=orange];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "III.31" [URL="/heath/III/31/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "III.21" [URL="/heath/III/21/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "I.43" [URL="/heath/I/43/", target="_top"];
     "I.46" [URL="/heath/I/46/", target="_top"];
     "III.32" [URL="/heath/III/32/", target="_top"];
     "III.24" [URL="/heath/III/24/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "II.6" [URL="/heath/II/6/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "I.17" [URL="/heath/I/17/", target="_top"];
     "I.32" [URL="/heath/I/32/", target="_top"];
     "I.18" [URL="/heath/I/18/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "III.def.11" [style="rounded,filled", fillcolor=orange, URL="/heath/III/def.11/", target="_top"];
     "III.3" [URL="/heath/III/3/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "III.36" [URL="/heath/III/36/", target="_top"];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "elem.3.1.p.1" [style="rounded,filled", fillcolor=orange];
     "III.37" [URL="/heath/III/37/", target="_top"];
     "I.19" [URL="/heath/I/19/", target="_top"];
     "I.6" [style="rounded,filled", fillcolor=orange, URL="/heath/I/6/", target="_top"];
     "III.5" [URL="/heath/III/5/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "IV.11" [style="rounded,filled", fillcolor=lightblue, URL="/heath/IV/11/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "III.26" [URL="/heath/III/26/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "III.29" [URL="/heath/III/29/", target="_top"];
     "IV.2" [URL="/heath/IV/2/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "IV.1" [style="rounded,filled", fillcolor=orange, URL="/heath/IV/1/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "I.14" [URL="/heath/I/14/", target="_top"];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.47" [URL="/heath/I/47/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "IV.10" -> "IV.5";
     "III.31" -> "III.22";
     "III.32" -> "III.22";
     "IV.10" -> "II.11";
     "I.8" -> "I.7";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.46" -> "I.29";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "IV.11" -> "IV.10";
     "I.27" -> "I.def.23";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.41" -> "I.34";
     "I.43" -> "I.34";
     "I.46" -> "I.34";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "III.3" -> "I.8";
     "III.24" -> "I.8";
     "III.37" -> "I.8";
     "I.10" -> "I.9";
     "IV.11" -> "I.9";
     "III.24" -> "III.10";
     "III.21" -> "III.20";
     "III.27" -> "III.20";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "I.3" -> "I.2";
     "III.29" -> "III.27";
     "IV.11" -> "III.27";
     "III.19" -> "III.18";
     "III.36" -> "III.18";
     "III.37" -> "III.18";
     "III.32" -> "III.19";
     "I.4" -> "I.cn.4";
     "III.37" -> "elem.3.16.p.1";
     "IV.2" -> "elem.3.16.p.1";
     "I.31" -> "I.23";
     "III.27" -> "I.23";
     "IV.2" -> "I.23";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "III.32" -> "III.31";
     "IV.5" -> "III.31";
     "I.7" -> "I.5";
     "I.19" -> "I.5";
     "III.3" -> "I.5";
     "III.20" -> "I.5";
     "III.31" -> "I.5";
     "IV.10" -> "I.5";
     "III.22" -> "III.21";
     "I.13" -> "I.11";
     "I.46" -> "I.11";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.47" -> "I.4";
     "III.26" -> "I.4";
     "III.29" -> "I.4";
     "IV.5" -> "I.4";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "III.3" -> "I.def.10";
     "III.31" -> "I.def.10";
     "II.6" -> "I.43";
     "I.47" -> "I.46";
     "II.6" -> "I.46";
     "II.11" -> "I.46";
     "IV.2" -> "III.32";
     "IV.10" -> "III.32";
     "III.26" -> "III.24";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "II.6" -> "I.36";
     "II.11" -> "II.6";
     "III.36" -> "II.6";
     "I.16" -> "I.15";
     "I.29" -> "I.15";
     "III.18" -> "I.17";
     "III.31" -> "I.17";
     "III.20" -> "I.32";
     "III.22" -> "I.32";
     "III.31" -> "I.32";
     "III.32" -> "I.32";
     "IV.2" -> "I.32";
     "IV.10" -> "I.32";
     "I.19" -> "I.18";
     "I.47" -> "I.41";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.43" -> "I.cn.3";
     "III.26" -> "III.def.11";
     "III.36" -> "III.3";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "III.5" -> "I.def.15";
     "III.10" -> "I.def.15";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "III.37" -> "III.36";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "I.34" -> "I.26";
     "III.3" -> "I.26";
     "I.36" -> "I.33";
     "III.10" -> "elem.3.1.p.1";
     "IV.10" -> "III.37";
     "III.18" -> "I.19";
     "IV.10" -> "I.6";
     "III.10" -> "III.5";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.43" -> "I.cn.2";
     "I.47" -> "I.cn.2";
     "I.16" -> "I.10";
     "IV.5" -> "I.10";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "I.18" -> "I.3";
     "I.17" -> "I.16";
     "I.18" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "III.27" -> "III.26";
     "IV.11" -> "III.26";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.46" -> "I.31";
     "II.6" -> "I.31";
     "IV.11" -> "III.29";
     "IV.11" -> "IV.2";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "IV.10" -> "IV.1";
     "I.29" -> "I.post.5";
     "I.47" -> "I.14";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "II.11" -> "I.47";
     "III.36" -> "I.47";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.41" -> "I.37";
   }



Required for
------------

:ref:`IV.12`