...
.. figure:: II.1.graphic.inverted.png
...
For let ``BF`` be drawn from ``B`` at right angles to ``BC``; [:ref:`I.11`] let ``BG`` be made equal to ``A``, [:ref:`I.3`] through ``G`` let ``GH`` be drawn parallel to ``BC``, [:ref:`I.31`] and through ``D``, ``E``, ``C`` let ``DK``, ``EL``, ``CH`` be drawn parallel to ``BG``.
...
.. container:: center
   ...
   and ``DL`` is the rectangle ``A``, ``DE``, for ``DK``, that is ``BG`` [:ref:`I.34`], is equal to ``A``.
...
.. graphviz::
   ...
   digraph {
     ...
     "II.1" [style="rounded,filled", fillcolor=lightblue, URL="/heath/II/1/", target="_top"];
