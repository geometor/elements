...
.. figure:: II.10.graphic.inverted.png
...
.. container:: center
   ...
   the angles ``CEF``, ``EFD`` are equal to two right angles; [:ref:`I.29`] therefore the angles ``FEB``, ``EFD`` are less than two right angles.


But straight lines produced from angles less than two right angles meet; [:ref:`I.post.5`]
...
Then, since ``AC`` is equal to ``CE``, the angle ``EAC`` is also equal to the angle ``AEC``; [:ref:`I.5`] and the angle at ``C`` is right;


.. container:: center

   therefore each of the angles ``EAC``, ``AEC`` is half a right angle. [:ref:`I.32`]
...
And, since the angle ``EBC`` is half a right angle, the angle ``DBG`` is also half a right angle. [:ref:`I.15`]
...
for it is equal to the angle ``DCE``, they being alternate; [:ref:`I.29`]


.. container:: center

   therefore the remaining angle ``DGB`` is half a right angle; [:ref:`I.32`]
...
.. container:: center
   ...
   so that the side ``BD`` is also equal to the side ``GD``. [:ref:`I.6`]


Again, since the angle ``EGF`` is half a right angle, and the angle at ``F`` is right, for it is equal to the opposite angle, the angle at ``C``, [:ref:`I.34`]


.. container:: center

   the remaining angle ``FEG`` is half a right angle; [:ref:`I.32`] therefore the angle ``EGF`` is equal to the angle ``FEG``, so that the side ``GF`` is also equal to the side ``EF``. [:ref:`I.6`]


Now, since the square on ``EC`` is equal to the square on ``CA``, the squares on ``EC``, ``CA`` are double of the square on ``CA``.

But the square on ``EA`` is equal to the squares on ``EC``, ``CA``; [:ref:`I.47`] therefore the square on ``EA`` is double of the square on ``AC``. [:ref:`I.cn.1`]
...
But the square on ``EG`` is equal to the squares on ``GF``, ``FE``; [:ref:`I.47`] therefore the square on ``EG`` is double of the square on ``EF``.

And ``EF`` is equal to ``CD``; [:ref:`I.34`]
...
And the square on ``AG`` is equal to the squares on ``AE``, ``EG``; [:ref:`I.47`] therefore the square on ``AG`` is double of the squares on ``AC``, ``CD``. But the squares on ``AD``, ``DG`` are equal to the square on ``AG``; [:ref:`I.47`] therefore the squares on ``AD``, ``DG`` are double of the squares on ``AC``, ``CD``.
...
.. graphviz::
   ...
   digraph {
     ...
     "II.10" [style="rounded,filled", fillcolor=lightblue, URL="/heath/II/10/", target="_top"];
