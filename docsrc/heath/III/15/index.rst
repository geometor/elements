...
Then, since ``BC`` is nearer to the centre and ``FG`` more remote, ``EK`` is greater than ``EH``. [:ref:`III.def.5`]
...
.. container:: center
   ...
   ``BC`` is also equal to ``MN``. [:ref:`III.14`]
...
But ``ME``, ``EN`` are greater than ``MN``, [:ref:`I.20`] and ``MN`` is equal to ``BC``;
...
And, since the two sides ``ME``, ``EN`` are equal to the two sides ``FE``, ``EG``, and the angle ``MEN`` greater than the angle ``FEG``, therefore the base ``MN`` is greater than the base ``FG``. [:ref:`I.24`]
...
.. graphviz::
   ...
   digraph {
     ...
     "III.15" [style="rounded,filled", fillcolor=lightblue, URL="/heath/III/15/", target="_top"];
     ...
     "III.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/III/def.5/", target="_top"];
     ...
     "III.14" [URL="/heath/III/14/", target="_top"];
