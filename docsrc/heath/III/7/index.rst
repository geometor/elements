...
But the angle ``BEF`` is also greater than the angle ``CEF``; therefore the base ``BF`` is greater than the base ``CF``. [:ref:`I.24`]
...
For on the straight line ``EF``, and at the point ``E`` on it, let the angle ``FEH`` be constructed equal to the angle ``GEF`` [:ref:`I.23`], and let ``FH`` be joined.
...
.. container:: center
   ...
   therefore the base ``FG`` is equal to the base ``FH``. [:ref:`I.4`]
...
.. graphviz::
   ...
   digraph {
     ...
     "III.7" [style="rounded,filled", fillcolor=lightblue, URL="/heath/III/7/", target="_top"];
