...
:type: prop
:tags: line, circle
:dependencies: I.8, III.18, III.36, elem.3.16.p.1
...
Thus the angle ``FED`` is right. [:ref:`III.18`]

Now, since ``DE`` touches the circle ``ABC``, and ``DCA`` cuts it, the rectangle ``AD``, ``DC`` is equal to the square on ``DE``. [:ref:`III.36`]
...
.. container:: center
   ...
   therefore the angle ``DEF`` is equal to the angle ``DBF``. [:ref:`I.8`]
...
And ``FB`` produced is a diameter; and the straight line drawn at right angles to the diameter of a circle, from its extremity, touches the circle; [:ref:`elem.3.16.p.1`]
...
.. graphviz::
   ...
   digraph {
     ...
     "III.37" [style="rounded,filled", fillcolor=lightblue, URL="/heath/III/37/", target="_top"];
