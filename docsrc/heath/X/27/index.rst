...
.. figure:: X.27.graphic.inverted.png
...
Let two rational straight lines A, B commensurable in square only be set out; let C be taken a mean proportional between A, B, [:ref:`VI.13`] and let it be contrived that,


.. container:: center

   as A is to B, so is C to D. [:ref:`VI.12`]


Then, since A, B are rational and commensurable in square only, the rectangle A, B, that is, the square on C [:ref:`VI.17`], is medial. [:ref:`X.21`]

Therefore C is medial. [:ref:`X.21`]

And since, as A is to B, so is C to D, and A, B are commensurable in square only, therefore C, D are also commensurable in square only. [:ref:`X.11`]
...
For since, as A is to B, so is C to D, therefore, alternately, as A is to C, so is B to D. [:ref:`V.16`]
...
.. graphviz::
   ...
   digraph {
     ...
     "X.27" [style="rounded,filled", fillcolor=lightblue, URL="/heath/X/27/", target="_top"];
