:order: 55
:number: 438
:type: None
:tags: line
:dependencies: I.18, VI.1, X.11, X.12, X.13, X.15, X.17, X.19, X.21, X.37, elem.10.def.2.2




.. figure:: X.55.graphic.inverted.png

.. _X.55:

X.55
====

   If an area be contained by a rational straight line and the second binomial, the side
    of the area is the irrational straight line which is called a first bimedial.

For let the area ABCD be contained by the rational straight line AB and the second binomial AD; I say that the side
of the area AC is a first bimedial straight line.

For, since AD is a second binomial straight line, let it be divided into its terms at E, so that AE is the greater term; therefore AE, ED are rational straight lines commensurable in square only, the square on AE is greater than the square on ED by the square on a straight line commensurable with AE, and the lesser term ED is commensurable in length with AB. [:ref:`elem.10.def.2.2`]

Let ED be bisected at F, and let there be applied to AE the rectangle AG, GE equal to the square on EF and deficient by a square figure; therefore AG is commensurable in length with GE. [:ref:`X.17`]

Through G, E, F let GH, EK, FL be drawn parallel to AB, CD, let the square SN be constructed equal to the parallelogram AH, and the square NQ equal to GK, and let them be placed so that MN is in a straight line with NO; therefore RN is also in a straight line with NP.

Let the square SQ be completed.

It is then manifest from what was proved before that MR is a mean proportional between SN, NQ and is equal to EL, and that MO is the side
of the area AC.

It is now to be proved that MO is a first bimedial straight line.

Since AE is incommensurable in length with ED, while ED is commensurable with AB, therefore AE is incommensurable with AB. [:ref:`X.13`]

And, since AG is commensurable with EG, AE is also commensurable with each of the straight lines AG, GE. [:ref:`X.15`]

But AE is incommensurable in length with AB; therefore AG, GE are also incommensurable with AB. [:ref:`X.13`]

Therefore BA, AG and BA, GE are pairs of rational straight lines commensurable in square only; so that each of the rectangles AH, GK is medial. [:ref:`X.21`]

Hence each of the squares SN, NQ is medial.

Therefore MN, NO are also medial.

And, since AG is commensurable in length with GE, AH is also commensurable with GK, [:ref:`VI.1`. :ref:`X.11`] that is, SN is commensurable with NQ, that is, the square on MN with the square on NO.

And, since AE is incommensurable in length with ED, while AE is commensurable with AG, and ED is commensurable with EF, therefore AG is incommensurable with EF; [:ref:`X.13`] so that AH is also incommensurable with EL, that is, SN is incommensurable with MR, that is, PN with NR, [:ref:`VI.1`, :ref:`X.11`] that is, MN is incommensurable in length with NO.

But MN, NO were proved to be both medial and commensurable in square; therefore MN, NO are medial straight lines commensurable in square only.

I say next that they also contain a rational rectangle.

For, since DE is, by hypothesis, commensurable with each of the straight lines AB, EF, therefore EF is also commensurable with EK. [:ref:`X.12`]

And each of them is rational; therefore EL, that is, MR is rational, [:ref:`X.19`] and MR is the rectangle MN, NO.

But, if two medial straight lines commensurable in square only and containing a rational rectangle be added together, the whole is irrational and is called a first bimedial straight line. [:ref:`X.37`]

Therefore MO is a first bimedial straight line. Q. E. D.
39. Therefore BA, AG and BA, GE are pairs of rational straight lines commensurable in square only. The text has Therefore BA, AG, GE are rational straight lines commensurable in square only,
which I have altered because it would naturally convey the impression that any two of the three straight lines are commensurable in square only, whereas AG, GE are commensurable in length (:ref:`I.18`), and it is only the other two pairs which are commensurable in square only.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "VII.12" [URL="/heath/VII/12/", target="_top"];
     "III.22" [URL="/heath/III/22/", target="_top"];
     "X.7" [URL="/heath/X/7/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "VII.13" [URL="/heath/VII/13/", target="_top"];
     "VII.16" [URL="/heath/VII/16/", target="_top"];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "V.2" [style="rounded,filled", fillcolor=orange, URL="/heath/V/2/", target="_top"];
     "VII.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.15/", target="_top"];
     "VII.18" [URL="/heath/VII/18/", target="_top"];
     "V.22" [URL="/heath/V/22/", target="_top"];
     "III.20" [URL="/heath/III/20/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "elem.5.7.p.1" [style="rounded,filled", fillcolor=orange];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "V.4" [URL="/heath/V/4/", target="_top"];
     "X.36" [URL="/heath/X/36/", target="_top"];
     "VII.15" [URL="/heath/VII/15/", target="_top"];
     "VII.def.12" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.12/", target="_top"];
     "X.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.1/", target="_top"];
     "VII.2" [URL="/heath/VII/2/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "elem.7.5 elem.7.6" [style="rounded,filled", fillcolor=orange];
     "VII.3" [URL="/heath/VII/3/", target="_top"];
     "X.8" [URL="/heath/X/8/", target="_top"];
     "VII.10" [URL="/heath/VII/10/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "III.31" [URL="/heath/III/31/", target="_top"];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "VII.20" [URL="/heath/VII/20/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "III.21" [URL="/heath/III/21/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "X.6" [URL="/heath/X/6/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "VII.9" [URL="/heath/VII/9/", target="_top"];
     "II.4" [URL="/heath/II/4/", target="_top"];
     "I.43" [URL="/heath/I/43/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "I.46" [URL="/heath/I/46/", target="_top"];
     "X.16" [URL="/heath/X/16/", target="_top"];
     "X.37" [URL="/heath/X/37/", target="_top"];
     "VII.19" [URL="/heath/VII/19/", target="_top"];
     "VII.4" [URL="/heath/VII/4/", target="_top"];
     "elem.7.2.p.1" [style="rounded,filled", fillcolor=orange];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "elem.10.def.2.2" [style="rounded,filled", fillcolor=orange];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "V.3" [URL="/heath/V/3/", target="_top"];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "V.12" [URL="/heath/V/12/", target="_top"];
     "X.19" [URL="/heath/X/19/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "VII.1" [URL="/heath/VII/1/", target="_top"];
     "I.17" [URL="/heath/I/17/", target="_top"];
     "elem.7.13 elem.7.def.20" [style="rounded,filled", fillcolor=orange];
     "I.32" [URL="/heath/I/32/", target="_top"];
     "V.14" [URL="/heath/V/14/", target="_top"];
     "I.18" [URL="/heath/I/18/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "X.15" [URL="/heath/X/15/", target="_top"];
     "X.13" [URL="/heath/X/13/", target="_top"];
     "VII.35" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/35/", target="_top"];
     "I.38" [URL="/heath/I/38/", target="_top"];
     "V.16" [URL="/heath/V/16/", target="_top"];
     "V.15" [URL="/heath/V/15/", target="_top"];
     "VIII.4" [URL="/heath/VIII/4/", target="_top"];
     "elem.7.def.20 elem.7.13" [style="rounded,filled", fillcolor=orange];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "V.21" [URL="/heath/V/21/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "VII.21" [URL="/heath/VII/21/", target="_top"];
     "II.5" [URL="/heath/II/5/", target="_top"];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "X.5" [URL="/heath/X/5/", target="_top"];
     "VII.34" [URL="/heath/VII/34/", target="_top"];
     "VII.def.20" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.20/", target="_top"];
     "elem.6.19.p.1" [style="rounded,filled", fillcolor=orange];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "X.11" [URL="/heath/X/11/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "I.6" [style="rounded,filled", fillcolor=orange, URL="/heath/I/6/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "X.21" [URL="/heath/X/21/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "X.12" [URL="/heath/X/12/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "X.17" [URL="/heath/X/17/", target="_top"];
     "X.55" [style="rounded,filled", fillcolor=lightblue, URL="/heath/X/55/", target="_top"];
     "IV.1" [style="rounded,filled", fillcolor=orange, URL="/heath/IV/1/", target="_top"];
     "X.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.4/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "VI.1" [URL="/heath/VI/1/", target="_top"];
     "I.14" [URL="/heath/I/14/", target="_top"];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.47" [URL="/heath/I/47/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "VII.33" [URL="/heath/VII/33/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "VII.17" [URL="/heath/VII/17/", target="_top"];
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "VII.15" -> "VII.12";
     "VII.20" -> "VII.12";
     "III.31" -> "III.22";
     "X.11" -> "X.7";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.46" -> "I.29";
     "II.4" -> "I.29";
     "I.8" -> "I.7";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "I.27" -> "I.def.23";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "I.43" -> "I.34";
     "I.46" -> "I.34";
     "II.4" -> "I.34";
     "VII.17" -> "VII.13";
     "VII.20" -> "VII.13";
     "VIII.4" -> "VII.13";
     "VII.18" -> "VII.16";
     "VII.21" -> "VII.16";
     "VII.33" -> "VII.16";
     "VII.34" -> "VII.16";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "I.10" -> "I.9";
     "V.3" -> "V.2";
     "VII.33" -> "VII.def.15";
     "VII.34" -> "VII.def.15";
     "VII.19" -> "VII.18";
     "X.5" -> "V.22";
     "X.6" -> "V.22";
     "X.12" -> "V.22";
     "III.21" -> "III.20";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "X.5" -> "elem.5.7.p.1";
     "X.6" -> "elem.5.7.p.1";
     "I.3" -> "I.2";
     "V.22" -> "V.4";
     "X.37" -> "X.36";
     "VII.16" -> "VII.15";
     "VII.1" -> "VII.def.12";
     "VII.21" -> "VII.def.12";
     "X.15" -> "X.def.1";
     "X.16" -> "X.def.1";
     "VII.3" -> "VII.2";
     "VII.4" -> "VII.2";
     "I.4" -> "I.cn.4";
     "VII.9" -> "elem.7.5 elem.7.6";
     "VII.10" -> "elem.7.5 elem.7.6";
     "VII.12" -> "elem.7.5 elem.7.6";
     "VII.33" -> "VII.3";
     "X.11" -> "X.8";
     "VII.13" -> "VII.10";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
     "X.13" -> "III.31";
     "I.31" -> "I.23";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "VII.21" -> "VII.20";
     "VII.34" -> "VII.20";
     "VIII.4" -> "VII.20";
     "I.7" -> "I.5";
     "II.4" -> "I.5";
     "III.20" -> "I.5";
     "III.31" -> "I.5";
     "III.22" -> "III.21";
     "I.13" -> "I.11";
     "I.46" -> "I.11";
     "X.7" -> "X.6";
     "X.11" -> "X.6";
     "X.12" -> "X.6";
     "X.17" -> "X.6";
     "X.36" -> "X.6";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.47" -> "I.4";
     "VII.10" -> "VII.9";
     "X.36" -> "II.4";
     "X.37" -> "II.4";
     "II.5" -> "I.43";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "III.31" -> "I.def.10";
     "I.47" -> "I.46";
     "II.4" -> "I.46";
     "II.5" -> "I.46";
     "X.36" -> "X.16";
     "X.37" -> "X.16";
     "X.55" -> "X.37";
     "VII.33" -> "VII.19";
     "VII.34" -> "VII.19";
     "VII.20" -> "VII.4";
     "VII.3" -> "elem.7.2.p.1";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "VII.19" -> "V.9";
     "X.6" -> "V.9";
     "X.55" -> "elem.10.def.2.2";
     "V.16" -> "elem.5.17 elem.5.18";
     "V.4" -> "V.3";
     "I.38" -> "I.36";
     "II.5" -> "I.36";
     "V.15" -> "V.12";
     "X.55" -> "X.19";
     "I.16" -> "I.15";
     "I.29" -> "I.15";
     "VII.2" -> "VII.1";
     "III.31" -> "I.17";
     "VIII.4" -> "elem.7.13 elem.7.def.20";
     "III.20" -> "I.32";
     "III.22" -> "I.32";
     "III.31" -> "I.32";
     "V.16" -> "V.14";
     "X.55" -> "I.18";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "I.47" -> "I.41";
     "VI.1" -> "I.41";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.43" -> "I.cn.3";
     "X.17" -> "X.15";
     "X.36" -> "X.15";
     "X.55" -> "X.15";
     "X.36" -> "X.13";
     "X.55" -> "X.13";
     "VIII.4" -> "VII.35";
     "VI.1" -> "I.38";
     "VI.1" -> "V.16";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "X.12" -> "VIII.4";
     "VIII.4" -> "elem.7.def.20 elem.7.13";
     "V.8" -> "V.def.4";
     "V.16" -> "V.21";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "VII.33" -> "VII.21";
     "VII.34" -> "VII.21";
     "X.17" -> "II.5";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VII.19" -> "V.7";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "X.8" -> "X.5";
     "X.11" -> "X.5";
     "X.12" -> "X.5";
     "VIII.4" -> "VII.34";
     "VII.12" -> "VII.def.20";
     "VII.13" -> "VII.def.20";
     "VII.17" -> "VII.def.20";
     "VII.20" -> "VII.def.20";
     "VII.33" -> "VII.def.20";
     "X.5" -> "VII.def.20";
     "X.6" -> "VII.def.20";
     "X.6" -> "elem.6.19.p.1";
     "I.34" -> "I.26";
     "I.36" -> "I.33";
     "X.19" -> "X.11";
     "X.21" -> "X.11";
     "X.36" -> "X.11";
     "X.55" -> "X.11";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "II.4" -> "I.6";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.43" -> "I.cn.2";
     "I.47" -> "I.cn.2";
     "X.55" -> "X.21";
     "I.16" -> "I.10";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "I.18" -> "I.3";
     "I.17" -> "I.16";
     "I.18" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.46" -> "I.31";
     "II.4" -> "I.31";
     "II.5" -> "I.31";
     "V.16" -> "V.11";
     "VI.1" -> "V.11";
     "X.6" -> "V.11";
     "X.12" -> "V.11";
     "X.13" -> "X.12";
     "X.17" -> "X.12";
     "X.55" -> "X.12";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "X.55" -> "X.17";
     "X.13" -> "IV.1";
     "X.19" -> "X.def.4";
     "X.21" -> "X.def.4";
     "X.36" -> "X.def.4";
     "X.37" -> "X.def.4";
     "I.29" -> "I.post.5";
     "X.19" -> "VI.1";
     "X.21" -> "VI.1";
     "X.55" -> "VI.1";
     "I.47" -> "I.14";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "X.13" -> "I.47";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "VII.34" -> "VII.33";
     "I.41" -> "I.37";
     "VII.18" -> "VII.17";
     "VII.19" -> "VII.17";
     "VII.34" -> "VII.17";
   }



Required for
------------

:ref:`X.71`