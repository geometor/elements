:order: 18
:number: 395
:type: prop
:tags: line
:dependencies: X.13, X.16, X.6, elem.10.16.l.1




.. figure:: X.18.graphic.inverted.png

.. _X.18:

X.18
====

   If there be two unequal straight lines, and to the greater there be applied a parallelogram equal to the fourth part of the square on the less and deficient by a square figure, and if it divide it into parts which are incommensurable, the square on the greater will be greater than the square on the less by the square on a straight line incommensurable with the greater.

And, if the square on the greater be greater than the square on the less by the square on a straight line incommensurable with the greater, and if there be applied to the greater a parallelogram equal to the fourth part of the square on the less and deficient by a square figure, it divides it into parts which are incommensurable.

Let A, BC be two unequal straight lines, of which BC is the greater, and to BC let there be applied a parallelogram equal to the fourth part of the square on the less, A, and deficient by a square figure. Let this be the rectangle BD, DC, [cf. :ref:`elem.10.16.l.1`]

and let BD be incommensurable in length with DC; I say that the square on BC is greater than the square on A by the square on a straight line incommensurable with BC.

For, with the same construction as before, we can prove similarly that the square on BC is greater than the square on A by the square on FD.

It is to be proved that BC is incommensurable in length with DF.

Since BD is incommensurable in length with DC, therefore BC is also incommensurable in length with CD. [:ref:`X.16`]

But DC is commensurable with the sum of BF, DC; [:ref:`X.6`] therefore BC is also incommensurable with the sum of BF, DC; [:ref:`X.13`] so that BC is also incommensurable in length with the remainder FD. [:ref:`X.16`]

And the square on BC is greater than the square on A by the square on FD; therefore the square on BC is greater than the square on A by the square on a straight line incommensurable with BC.

Again, let the square on BC be greater than the square on A by the square on a straight line incommensurable with BC, and let there be applied to BC a parallelogram equal to the fourth part of the square on A and deficient by a square figure. Let this be the rectangle BD, DC.

It is to be proved that BD is incommensurable in length with DC.

For, with the same construction, we can prove similarly that the square on BC is greater than the square on A by the square on FD.

But the square on BC is greater than the square on A by the square on a straight line incommensurable with BC; therefore BC is incommensurable in length with FD. so that BC is also incommensurable with the remainder, the sum of BF, DC. [:ref:`X.16`]

But the sum of BF, DC is commensurable in length with DC; [:ref:`X.6`] therefore BC is also incommensurable in length with DC, [:ref:`X.13`] so that, separando, BD is also incommensurable in length with DC. [:ref:`X.16`]

Therefore etc.

LEMMA.

[Since it has been proved that straight lines commensurable in length are always commensurable in square also, while those commensurable in square are not always commensurable in length also, but can of course be either commensurable or incommensurable in length, it is manifest that, if any straight line be commensurable in length with a given rational straight line, it is called rational and commensurable with the other not only in length but in square also, since straight lines commensurable in length are always commensurable in square also. 

But, if any straight line be commensurable in square with a given rational straight line, then, if it is also commensurable in length with it, it is called in this case also rational and commensurable with it both in length and in square; but, if again any straight line, being commensurable in square with a given rational straight line, be incommensurable in length with it, it is called in this case also rational but commensurable in square only.]


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "VII.1" [URL="/heath/VII/1/", target="_top"];
     "VII.9" [URL="/heath/VII/9/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "III.20" [URL="/heath/III/20/", target="_top"];
     "X.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.1/", target="_top"];
     "III.31" [URL="/heath/III/31/", target="_top"];
     "X.13" [URL="/heath/X/13/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "I.47" [URL="/heath/I/47/", target="_top"];
     "elem.7.5 elem.7.6" [style="rounded,filled", fillcolor=orange];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "I.14" [URL="/heath/I/14/", target="_top"];
     "VII.13" [URL="/heath/VII/13/", target="_top"];
     "VII.2" [URL="/heath/VII/2/", target="_top"];
     "V.3" [URL="/heath/V/3/", target="_top"];
     "elem.7.13 elem.7.def.20" [style="rounded,filled", fillcolor=orange];
     "VII.12" [URL="/heath/VII/12/", target="_top"];
     "VII.def.12" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.12/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "VII.10" [URL="/heath/VII/10/", target="_top"];
     "elem.7.2.p.1" [style="rounded,filled", fillcolor=orange];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "VII.18" [URL="/heath/VII/18/", target="_top"];
     "IV.1" [style="rounded,filled", fillcolor=orange, URL="/heath/IV/1/", target="_top"];
     "I.17" [URL="/heath/I/17/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "VII.21" [URL="/heath/VII/21/", target="_top"];
     "VII.20" [URL="/heath/VII/20/", target="_top"];
     "VII.35" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/35/", target="_top"];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "X.16" [URL="/heath/X/16/", target="_top"];
     "I.32" [URL="/heath/I/32/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "III.22" [URL="/heath/III/22/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "VII.def.20" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.20/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "elem.7.def.20 elem.7.13" [style="rounded,filled", fillcolor=orange];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "V.22" [URL="/heath/V/22/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "X.6" [URL="/heath/X/6/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "VII.17" [URL="/heath/VII/17/", target="_top"];
     "VII.3" [URL="/heath/VII/3/", target="_top"];
     "elem.10.16.l.1" [style="rounded,filled", fillcolor=orange];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "X.18" [style="rounded,filled", fillcolor=lightblue, URL="/heath/X/18/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "X.12" [URL="/heath/X/12/", target="_top"];
     "elem.6.19.p.1" [style="rounded,filled", fillcolor=orange];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "I.46" [URL="/heath/I/46/", target="_top"];
     "VII.33" [URL="/heath/VII/33/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "VII.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.15/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "VII.19" [URL="/heath/VII/19/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "VII.16" [URL="/heath/VII/16/", target="_top"];
     "V.4" [URL="/heath/V/4/", target="_top"];
     "elem.5.7.p.1" [style="rounded,filled", fillcolor=orange];
     "VII.4" [URL="/heath/VII/4/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "VIII.4" [URL="/heath/VIII/4/", target="_top"];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "X.5" [URL="/heath/X/5/", target="_top"];
     "VII.34" [URL="/heath/VII/34/", target="_top"];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "V.2" [style="rounded,filled", fillcolor=orange, URL="/heath/V/2/", target="_top"];
     "III.21" [URL="/heath/III/21/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "VII.15" [URL="/heath/VII/15/", target="_top"];
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "III.31" -> "I.def.10";
     "I.13" -> "I.11";
     "I.46" -> "I.11";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "VII.2" -> "VII.1";
     "VII.10" -> "VII.9";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.22" -> "V.def.5";
     "III.21" -> "III.20";
     "X.16" -> "X.def.1";
     "X.13" -> "III.31";
     "X.18" -> "X.13";
     "V.20" -> "V.13";
     "X.6" -> "V.11";
     "X.12" -> "V.11";
     "X.13" -> "I.47";
     "VII.9" -> "elem.7.5 elem.7.6";
     "VII.10" -> "elem.7.5 elem.7.6";
     "VII.12" -> "elem.7.5 elem.7.6";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.47" -> "I.14";
     "VII.17" -> "VII.13";
     "VII.20" -> "VII.13";
     "VIII.4" -> "VII.13";
     "VII.3" -> "VII.2";
     "VII.4" -> "VII.2";
     "V.4" -> "V.3";
     "VIII.4" -> "elem.7.13 elem.7.def.20";
     "VII.15" -> "VII.12";
     "VII.20" -> "VII.12";
     "VII.1" -> "VII.def.12";
     "VII.21" -> "VII.def.12";
     "I.47" -> "I.41";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.20" -> "V.8";
     "I.31" -> "I.27";
     "I.41" -> "I.37";
     "I.35" -> "I.34";
     "I.37" -> "I.34";
     "I.41" -> "I.34";
     "I.46" -> "I.34";
     "VII.13" -> "VII.10";
     "VII.3" -> "elem.7.2.p.1";
     "I.3" -> "I.2";
     "I.27" -> "I.def.23";
     "VII.19" -> "VII.18";
     "X.13" -> "IV.1";
     "III.31" -> "I.17";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "VII.33" -> "VII.21";
     "VII.34" -> "VII.21";
     "VII.21" -> "VII.20";
     "VII.34" -> "VII.20";
     "VIII.4" -> "VII.20";
     "VIII.4" -> "VII.35";
     "I.10" -> "I.9";
     "X.18" -> "X.16";
     "III.20" -> "I.32";
     "III.22" -> "I.32";
     "III.31" -> "I.32";
     "V.8" -> "V.def.4";
     "I.4" -> "I.cn.4";
     "I.16" -> "I.15";
     "I.29" -> "I.15";
     "III.31" -> "III.22";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "VII.12" -> "VII.def.20";
     "VII.13" -> "VII.def.20";
     "VII.17" -> "VII.def.20";
     "VII.20" -> "VII.def.20";
     "VII.33" -> "VII.def.20";
     "X.5" -> "VII.def.20";
     "X.6" -> "VII.def.20";
     "I.7" -> "I.5";
     "III.20" -> "I.5";
     "III.31" -> "I.5";
     "VIII.4" -> "elem.7.def.20 elem.7.13";
     "I.34" -> "I.26";
     "I.32" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.46" -> "I.29";
     "X.5" -> "V.22";
     "X.6" -> "V.22";
     "X.12" -> "V.22";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.47" -> "I.cn.2";
     "X.12" -> "X.6";
     "X.18" -> "X.6";
     "V.22" -> "V.20";
     "V.20" -> "V.10";
     "VII.18" -> "VII.17";
     "VII.19" -> "VII.17";
     "VII.34" -> "VII.17";
     "VII.33" -> "VII.3";
     "X.18" -> "elem.10.16.l.1";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "V.10" -> "V.7";
     "VII.19" -> "V.7";
     "VII.19" -> "V.9";
     "X.6" -> "V.9";
     "I.31" -> "I.23";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "X.13" -> "X.12";
     "X.6" -> "elem.6.19.p.1";
     "I.37" -> "I.35";
     "I.16" -> "I.10";
     "I.47" -> "I.46";
     "VII.34" -> "VII.33";
     "I.8" -> "I.7";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "VII.33" -> "VII.def.15";
     "VII.34" -> "VII.def.15";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.47" -> "I.4";
     "VII.33" -> "VII.19";
     "VII.34" -> "VII.19";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "VII.18" -> "VII.16";
     "VII.21" -> "VII.16";
     "VII.33" -> "VII.16";
     "VII.34" -> "VII.16";
     "V.22" -> "V.4";
     "X.5" -> "elem.5.7.p.1";
     "X.6" -> "elem.5.7.p.1";
     "VII.20" -> "VII.4";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.46" -> "I.31";
     "X.12" -> "VIII.4";
     "V.8" -> "V.1";
     "I.17" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "X.12" -> "X.5";
     "VIII.4" -> "VII.34";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "V.3" -> "V.2";
     "III.22" -> "III.21";
     "I.29" -> "I.post.5";
     "VII.16" -> "VII.15";
   }



Required for
------------

:ref:`X.100`, :ref:`X.101`, :ref:`X.102`, :ref:`X.105`, :ref:`X.106`, :ref:`X.107`, :ref:`X.108`, :ref:`X.109`, :ref:`X.110`, :ref:`X.33`, :ref:`X.34`, :ref:`X.35`, :ref:`X.39`, :ref:`X.40`, :ref:`X.41`, :ref:`X.45`, :ref:`X.46`, :ref:`X.57`, :ref:`X.58`, :ref:`X.59`, :ref:`X.63`, :ref:`X.64`, :ref:`X.65`, :ref:`X.68`, :ref:`X.69`, :ref:`X.70`, :ref:`X.71`, :ref:`X.72`, :ref:`X.76`, :ref:`X.77`, :ref:`X.78`, :ref:`X.82`, :ref:`X.83`, :ref:`X.84`, :ref:`X.94`, :ref:`X.95`, :ref:`X.96`, :ref:`XIII.11`, :ref:`XIII.16`, :ref:`XIII.18`