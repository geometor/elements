:order: 54
:number: 437
:type: prop
:tags: line
:dependencies: II.14, VI.1, VI.17, X.11, X.12, X.13, X.15, X.17, X.19, X.36, X.def.2.1, X.def.4




.. figure:: X.54.graphic.inverted.png

.. _X.54:

X.54
====

   If an area be contained by a rational straight line and the first binomial, the side
    of the area is the irrational straight line which is called binomial.

For let the area AC be contained by the rational straight line AB and the first binomial AD; I say that the side
of the area AC is the irrational straight line which is called binomial.

For, since AD is a first binomial straight line, let it be divided into its terms at E, and let AE be the greater term.

It is then manifest that AE, ED are rational straight lines commensurable in square only, the square on AE is greater than the square on ED by the square on a straight line commensurable with AE, and AE is commensurable in length with the rational straight line AB set out. [:ref:`X.def.2.1`]

Let ED be bisected at the point F.

Then, since the square on AE is greater than the square on ED by the square on a straight line commensurable with AE, therefore, if there be applied to the greater AE a parallelogram equal to the fourth part of the square on the less, that is, to the square on EF, and deficient by a square figure, it divides it into commensurable parts. [:ref:`X.17`]

Let then the rectangle AG, GE equal to the square on EF be applied to AE; therefore AG is commensurable in length with EG.

Let GH, EK, FL be drawn from G, E, F parallel to either of the straight lines AB, CD; let the square SN be constructed equal to the parallelogram AH, and the square NQ equal to GK, [:ref:`II.14`] and let them be placed so that MN is in a straight line with NO; therefore RN is also in a straight line with NP.

And let the parallelogram SQ be completed; therefore SQ is a square. [Lemma]

Now, since the rectangle AG, GE is equal to the square on EF, therefore, as AG is to EF, so is FE to EG; [:ref:`VI.17`] therefore also, as AH is to EL, so is EL to KG; [:ref:`VI.1`] therefore EL is a mean proportional between AH, GK.

But AH is equal to SN, and GK to NQ; therefore EL is a mean proportional between SN, NQ.

But MR is also a mean proportional between the same SN, NQ; [Lemma] therefore EL is equal to MR, so that it is also equal to PO.

But AH, GK are also equal to SN, NQ; therefore the whole AC is equal to the whole SQ, that is, to the square on MO; therefore MO is the side
of AC.

I say next that MO is binomial.

For, since AG is commensurable with GE, therefore AE is also commensurable with each of the straight lines AG, GE. [:ref:`X.15`]

But AE is also, by hypothesis, commensurable with AB; therefore AG, GE are also commensurable with AB. [:ref:`X.12`]

And AB is rational; therefore each of the straight lines AG, GE is also rational; therefore each of the rectangles AH, GK is rational, [:ref:`X.19`] and AH is commensurable with GK.

But AH is equal to SN, and GK to NQ; therefore SN, NQ, that is, the squares on MN, NO, are rational and commensurable.

And, since AE is incommensurable in length with ED, while AE is commensurable with AG, and DE is commensurable with EF, therefore AG is also incommensurable with EF, [:ref:`X.13`] so that AH is also incommensurable with EL. [:ref:`VI.1`, :ref:`X.11`]

But AH is equal to SN, and EL to MR; therefore SN is also incommensurable with MR.

But, as SN is to MR, so is PN to NR; [:ref:`VI.1`] therefore PN is incommensurable with NR. [:ref:`X.11`]

But PN is equal to MN, and NR to NO; therefore MN is incommensurable with NO.

And the square on MN is commensurable with the square on NO, and each is rational; therefore MN, NO are rational straight lines commensurable in square only.

Therefore MO is binomial [:ref:`X.36`] and the side
of AC. Q. E. D.
2. side.
I use the word side
in the sense explained in the note on :ref:`X.def.4` (P. 13 above), i.e. as short for side of a square equal to.
The Greek is ἡ τὸ χωίον δυναμένη.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "X.15" [URL="/heath/X/15/", target="_top"];
     "elem.7.def.20 elem.7.13" [style="rounded,filled", fillcolor=orange];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "VII.12" [URL="/heath/VII/12/", target="_top"];
     "IV.1" [style="rounded,filled", fillcolor=orange, URL="/heath/IV/1/", target="_top"];
     "VII.34" [URL="/heath/VII/34/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "VII.16" [URL="/heath/VII/16/", target="_top"];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "V.7.p.1" [style="rounded,filled", fillcolor=orange];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "V.14" [URL="/heath/V/14/", target="_top"];
     "X.def.2.1" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.2.1/", target="_top"];
     "III.20" [URL="/heath/III/20/", target="_top"];
     "X.6" [URL="/heath/X/6/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "I.43" [URL="/heath/I/43/", target="_top"];
     "I.30" [URL="/heath/I/30/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "III.22" [URL="/heath/III/22/", target="_top"];
     "VII.13" [URL="/heath/VII/13/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "II.14" [URL="/heath/II/14/", target="_top"];
     "V.4" [URL="/heath/V/4/", target="_top"];
     "VIII.4" [URL="/heath/VIII/4/", target="_top"];
     "X.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.4/", target="_top"];
     "X.54" [style="rounded,filled", fillcolor=lightblue, URL="/heath/X/54/", target="_top"];
     "VI.1" [URL="/heath/VI/1/", target="_top"];
     "VI.14" [URL="/heath/VI/14/", target="_top"];
     "VII.35" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/35/", target="_top"];
     "I.14" [URL="/heath/I/14/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "VII.2.p.1" [style="rounded,filled", fillcolor=orange];
     "X.17" [URL="/heath/X/17/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "VII.15" [URL="/heath/VII/15/", target="_top"];
     "VII.def.20" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.20/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "X.19" [URL="/heath/X/19/", target="_top"];
     "X.36" [URL="/heath/X/36/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "V.21" [URL="/heath/V/21/", target="_top"];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "V.3" [URL="/heath/V/3/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "X.16" [URL="/heath/X/16/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "I.17" [URL="/heath/I/17/", target="_top"];
     "I.45" [URL="/heath/I/45/", target="_top"];
     "VII.21" [URL="/heath/VII/21/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "I.38" [URL="/heath/I/38/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "VII.def.12" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.12/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "elem.7.13 elem.7.def.20" [style="rounded,filled", fillcolor=orange];
     "I.44" [URL="/heath/I/44/", target="_top"];
     "VII.20" [URL="/heath/VII/20/", target="_top"];
     "I.46" [URL="/heath/I/46/", target="_top"];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "V.16" [URL="/heath/V/16/", target="_top"];
     "VII.33" [URL="/heath/VII/33/", target="_top"];
     "VI.16" [URL="/heath/VI/16/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "I.32" [URL="/heath/I/32/", target="_top"];
     "X.11" [URL="/heath/X/11/", target="_top"];
     "elem.7.5 elem.7.6" [style="rounded,filled", fillcolor=orange];
     "VII.9" [URL="/heath/VII/9/", target="_top"];
     "I.47" [URL="/heath/I/47/", target="_top"];
     "VII.18" [URL="/heath/VII/18/", target="_top"];
     "X.12" [URL="/heath/X/12/", target="_top"];
     "X.8" [URL="/heath/X/8/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "II.5" [URL="/heath/II/5/", target="_top"];
     "I.42" [URL="/heath/I/42/", target="_top"];
     "VII.10" [URL="/heath/VII/10/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "VII.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/VII/def.15/", target="_top"];
     "II.4" [URL="/heath/II/4/", target="_top"];
     "VII.19" [URL="/heath/VII/19/", target="_top"];
     "VII.17" [URL="/heath/VII/17/", target="_top"];
     "V.12" [URL="/heath/V/12/", target="_top"];
     "VII.2" [URL="/heath/VII/2/", target="_top"];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "V.2" [style="rounded,filled", fillcolor=orange, URL="/heath/V/2/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "X.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/X/def.1/", target="_top"];
     "X.5" [URL="/heath/X/5/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "X.13" [URL="/heath/X/13/", target="_top"];
     "VI.19.p.1" [style="rounded,filled", fillcolor=orange];
     "VII.1" [URL="/heath/VII/1/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "III.31" [URL="/heath/III/31/", target="_top"];
     "X.7" [URL="/heath/X/7/", target="_top"];
     "VII.4" [URL="/heath/VII/4/", target="_top"];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "V.22" [URL="/heath/V/22/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "VII.3" [URL="/heath/VII/3/", target="_top"];
     "VI.17" [URL="/heath/VI/17/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "V.15" [URL="/heath/V/15/", target="_top"];
     "III.21" [URL="/heath/III/21/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "I.6" [style="rounded,filled", fillcolor=orange, URL="/heath/I/6/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "I.31" -> "I.23";
     "I.42" -> "I.23";
     "X.17" -> "X.15";
     "X.36" -> "X.15";
     "X.54" -> "X.15";
     "VIII.4" -> "elem.7.def.20 elem.7.13";
     "I.10" -> "I.9";
     "VII.15" -> "VII.12";
     "VII.20" -> "VII.12";
     "X.13" -> "IV.1";
     "VIII.4" -> "VII.34";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "VII.18" -> "VII.16";
     "VII.21" -> "VII.16";
     "VII.33" -> "VII.16";
     "VII.34" -> "VII.16";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.14" -> "V.7";
     "VII.19" -> "V.7";
     "V.16" -> "elem.5.17 elem.5.18";
     "X.5" -> "V.7.p.1";
     "X.6" -> "V.7.p.1";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "V.16" -> "V.14";
     "X.54" -> "X.def.2.1";
     "III.21" -> "III.20";
     "X.7" -> "X.6";
     "X.11" -> "X.6";
     "X.12" -> "X.6";
     "X.17" -> "X.6";
     "X.36" -> "X.6";
     "VI.14" -> "V.9";
     "VII.19" -> "V.9";
     "X.6" -> "V.9";
     "I.44" -> "I.43";
     "II.5" -> "I.43";
     "I.45" -> "I.30";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.42" -> "I.31";
     "I.44" -> "I.31";
     "I.46" -> "I.31";
     "II.4" -> "I.31";
     "II.5" -> "I.31";
     "III.31" -> "III.22";
     "VII.17" -> "VII.13";
     "VII.20" -> "VII.13";
     "VIII.4" -> "VII.13";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "III.31" -> "I.def.10";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "X.54" -> "II.14";
     "V.22" -> "V.4";
     "X.12" -> "VIII.4";
     "X.19" -> "X.def.4";
     "X.36" -> "X.def.4";
     "X.54" -> "X.def.4";
     "VI.14" -> "VI.1";
     "X.19" -> "VI.1";
     "X.54" -> "VI.1";
     "VI.16" -> "VI.14";
     "VIII.4" -> "VII.35";
     "I.45" -> "I.14";
     "I.47" -> "I.14";
     "VI.14" -> "I.14";
     "V.16" -> "V.11";
     "VI.1" -> "V.11";
     "VI.14" -> "V.11";
     "X.6" -> "V.11";
     "X.12" -> "V.11";
     "VII.3" -> "VII.2.p.1";
     "X.54" -> "X.17";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "I.43" -> "I.34";
     "I.45" -> "I.34";
     "I.46" -> "I.34";
     "II.4" -> "I.34";
     "VII.16" -> "VII.15";
     "VII.12" -> "VII.def.20";
     "VII.13" -> "VII.def.20";
     "VII.17" -> "VII.def.20";
     "VII.20" -> "VII.def.20";
     "VII.33" -> "VII.def.20";
     "X.5" -> "VII.def.20";
     "X.6" -> "VII.def.20";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "V.8" -> "V.def.4";
     "X.54" -> "X.19";
     "X.54" -> "X.36";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "V.16" -> "V.21";
     "I.3" -> "I.2";
     "V.4" -> "V.3";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.47" -> "I.4";
     "X.36" -> "X.16";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "III.31" -> "I.17";
     "II.14" -> "I.45";
     "VII.33" -> "VII.21";
     "VII.34" -> "VII.21";
     "I.30" -> "I.29";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.44" -> "I.29";
     "I.45" -> "I.29";
     "I.46" -> "I.29";
     "II.4" -> "I.29";
     "I.42" -> "I.38";
     "VI.1" -> "I.38";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "I.16" -> "I.15";
     "I.29" -> "I.15";
     "I.44" -> "I.15";
     "VII.1" -> "VII.def.12";
     "VII.21" -> "VII.def.12";
     "I.16" -> "I.10";
     "I.13" -> "I.11";
     "I.46" -> "I.11";
     "I.27" -> "I.def.23";
     "VIII.4" -> "elem.7.13 elem.7.def.20";
     "I.45" -> "I.44";
     "VII.21" -> "VII.20";
     "VII.34" -> "VII.20";
     "VIII.4" -> "VII.20";
     "I.47" -> "I.46";
     "II.4" -> "I.46";
     "II.5" -> "I.46";
     "I.34" -> "I.26";
     "VI.1" -> "V.16";
     "VII.34" -> "VII.33";
     "VI.17" -> "VI.16";
     "I.36" -> "I.33";
     "I.45" -> "I.33";
     "III.20" -> "I.32";
     "III.22" -> "I.32";
     "III.31" -> "I.32";
     "X.19" -> "X.11";
     "X.36" -> "X.11";
     "X.54" -> "X.11";
     "VII.9" -> "elem.7.5 elem.7.6";
     "VII.10" -> "elem.7.5 elem.7.6";
     "VII.12" -> "elem.7.5 elem.7.6";
     "VII.10" -> "VII.9";
     "II.14" -> "I.47";
     "X.13" -> "I.47";
     "VII.19" -> "VII.18";
     "X.13" -> "X.12";
     "X.17" -> "X.12";
     "X.54" -> "X.12";
     "X.11" -> "X.8";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "II.14" -> "II.5";
     "X.17" -> "II.5";
     "I.44" -> "I.42";
     "I.45" -> "I.42";
     "VII.13" -> "VII.10";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "VII.33" -> "VII.def.15";
     "VII.34" -> "VII.def.15";
     "X.36" -> "II.4";
     "VII.33" -> "VII.19";
     "VII.34" -> "VII.19";
     "VII.18" -> "VII.17";
     "VII.19" -> "VII.17";
     "VII.34" -> "VII.17";
     "V.15" -> "V.12";
     "VII.3" -> "VII.2";
     "VII.4" -> "VII.2";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.30" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.44" -> "I.cn.1";
     "I.45" -> "I.cn.1";
     "V.3" -> "V.2";
     "I.8" -> "I.7";
     "I.38" -> "I.36";
     "II.5" -> "I.36";
     "X.15" -> "X.def.1";
     "X.16" -> "X.def.1";
     "X.8" -> "X.5";
     "X.11" -> "X.5";
     "X.12" -> "X.5";
     "I.7" -> "I.5";
     "II.4" -> "I.5";
     "III.20" -> "I.5";
     "III.31" -> "I.5";
     "I.41" -> "I.37";
     "X.36" -> "X.13";
     "X.54" -> "X.13";
     "X.6" -> "VI.19.p.1";
     "VII.2" -> "VII.1";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.43" -> "I.cn.3";
     "I.17" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "X.13" -> "III.31";
     "X.11" -> "X.7";
     "VII.20" -> "VII.4";
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "X.5" -> "V.22";
     "X.6" -> "V.22";
     "X.12" -> "V.22";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.43" -> "I.cn.2";
     "I.45" -> "I.cn.2";
     "I.47" -> "I.cn.2";
     "I.29" -> "I.post.5";
     "I.44" -> "I.post.5";
     "VII.33" -> "VII.3";
     "X.54" -> "VI.17";
     "I.4" -> "I.cn.4";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "III.22" -> "III.21";
     "I.42" -> "I.41";
     "I.47" -> "I.41";
     "VI.1" -> "I.41";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "II.4" -> "I.6";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
   }



Required for
------------

:ref:`X.71`