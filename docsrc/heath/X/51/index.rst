...
:type: None
:tags: line
:dependencies: X.6, X.9, elem.10.6.p.1, elem.5.19.p.1




.. figure:: X.51.graphic.inverted.png
...
Let it be contrived that, as the number BA is to AC, so is the square on EF to the square on FG; [:ref:`elem.10.6.p.1`] therefore the square on EF is commensurable with the square on FG; [:ref:`X.6`] therefore FG is also rational.

Now, since BA has not to AC the ratio which a square number has to a square number, neither has the square on EF to the square on FG the ratio which a square number has to a square number; therefore EF is incommensurable in length with FG. [:ref:`X.9`]
...
Let then the squares on FG, H be equal to the square on EF; therefore, convertendo, as the number AB is to BC, so is the square on EF to the square on H. [:ref:`elem.5.19.p.1`]
...
Therefore EF is incommensurable in length with H; [:ref:`X.9`] therefore the square on EF is greater than the square on GF by the square on a straight line incommensurable with EF.
...
.. graphviz::
   ...
   digraph {
     ...
     "X.51" [style="rounded,filled", fillcolor=lightblue, URL="/heath/X/51/", target="_top"];
