:order: 13
:number: 602
:type: prop
:categories: construct
:dependencies: I.4, III.1, III.31, IV.2, VI.1, VI.17, VI.4, VI.8, VI.8.p.1, XI.12, XI.def.3, XIII.12




.. figure:: XIII.13.graphic.inverted.png

.. _XIII.13:

XIII.13
=======

   To construct a pyramid, to comprehend it in a given sphere, and to prove that the square on the diameter of the sphere is one and a half times the square on the side of the pyramid.

Let the diameter AB of the given sphere be set out, and let it be cut at the point C so that AC is double of CB; let the semicircle ADB be described on AB, let CD be drawn from the point C at right angles to AB, and let DA be joined; let the circle EFG which has its radius equal to DC be set out, let the equilateral triangle EFG be inscribed in the circle EFG, [:ref:`IV.2`] let the centre of the circle, the point H, be taken, [:ref:`III.1`] let EH, HF, HG be joined; from the point H let HK be set up at right angles to the plane of the circle EFG, [:ref:`XI.12`] let HK equal to the straight line AC be cut off from HK, and let KE, KF, KG be joined.

Now, since KH is at right angles to the plane of the circle EFG, therefore it will also make right angles with all the straight lines which meet it and are in the plane of the circle EFG. [:ref:`XI.def.3`]

But each of the straight lines HE, HF, HG meets it: therefore HK is at right angles to each of the straight lines HE, HF, HG.

And, since AC is equal to HK, and CD to HE, and they contain right angles, therefore the base DA is equal to the base KE. [:ref:`I.4`]

For the same reason each of the straight lines KF, KG is also equal to DA; therefore the three straight lines KE, KF, KG are equal to one another.

And, since AC is double of CB, therefore AB is triple of BC.

But, as AB is to BC, so is the square on AD to the square on DC, as will be proved afterwards.

Therefore the square on AD is triple of the square on DC.

But the square on FE is also triple of the square on EH, [:ref:`XIII.12`] and DC is equal to EH; therefore DA is also equal to EF.

But DA was proved equal to each of the straight lines KE, KF, KG; therefore each of the straight lines EF, FG, GE is also equal to each of the straight lines KE, KF, KG; therefore the four triangles EFG, KEF, KFG, KEG are equilateral.

Therefore a pyramid has been constructed out of four equilateral triangles, the triangle EFG being its base and the point K its vertex.

It is next required to comprehend it in the given sphere and to prove that the square on the diameter of the sphere is one and a half times the square on the side of the pyramid.

For let the straight line HL be produced in a straight line with KH, and let HL be made equal to CB.

Now, since, as AC is to CD, so is CD to CB, [:ref:`VI.8.p.1`] while AC is equal to KH, CD to HE, and CB to HL, therefore, as KH is to HE, so is EH to HL; therefore the rectangle KH, HL is equal to the square on EH. [:ref:`VI.17`]

And each of the angles KHE. EHL is right; therefore the semicircle described on KL will pass through E also. [cf. :ref:`VI.8`, :ref:`III.31`.]

If then, KL remaining fixed, the semicircle be carried round and restored to the same position from which it began to be moved, it will also pass through the points F, G, since, if FL, LG be joined, the angles at F, G similarly become right angles; and the pyramid will be comprehended in the given sphere.

For KL, the diameter of the sphere, is equal to the diameter AB of the given sphere, inasmuch as KH was made equal to AC, and HL to CB.

I say next that the square on the diameter of the sphere is one and a half times the square on the side of the pyramid

For, since AC is double of CB, therefore AB is triple of BC; and, convertendo, BA is one and a half times AC.

But, as BA is to AC, so is the square on BA to the square on AD.

Therefore the square on BA is also one and a half times the square on AD.

And BA is the diameter of the given sphere, and AD is equal to the side of the pyramid.

Therefore the square on the diameter of the sphere is one and a half times the square on the side of the pyramid. Q. E. D.


.. _XIII.13.l.1:


**XIII.13.l.1**


It is to be proved that, as AB is to BC, so is the square on AD to the square on DC. 
For let the figure of the semicircle be set out, let DB be joined, let the square EC be described on AC, and let the parallelogram FB be completed. 
       
Since then, because the triangle DAB is equiangular with the triangle DAC, as BA is to AD, so is DA to AC, [:ref:`VI.8`, :ref:`VI.4`] therefore the rectangle BA, AC is equal to the square on AD. [:ref:`VI.17`] 
And since, as AB is to BC, so is EB to BF, [:ref:`VI.1`] and EB is the rectangle BA, AC, for EA is equal to AC, and BF is the rectangle AC, CB, therefore, as AB is to BC, so is the rectangle BA, AC to the rectangle AC, CB. 
And the rectangle BA, AC is equal to the square on AD, and the rectangle AC, CB to the square on DC, for the perpendicular DC is a mean proportional between the segments AC, CB of the base, because the angle ADB is right. [:ref:`VI.8.p.1`] 
Therefore, as AB is to BC, so is the square on AD to the square on DC. Q. E. D.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "III.16.p.1" [style="rounded,filled", fillcolor=orange];
     "VI.def.1" [style="rounded,filled", fillcolor=orange, URL="/heath/VI/def.1/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "IV.15.p.1" [style="rounded,filled", fillcolor=orange];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "V.14" [URL="/heath/V/14/", target="_top"];
     "I.19" [URL="/heath/I/19/", target="_top"];
     "III.20" [URL="/heath/III/20/", target="_top"];
     "III.19" [URL="/heath/III/19/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "XIII.12" [URL="/heath/XIII/12/", target="_top"];
     "III.22" [URL="/heath/III/22/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "V.4" [URL="/heath/V/4/", target="_top"];
     "XI.def.3" [style="rounded,filled", fillcolor=orange, URL="/heath/XI/def.3/", target="_top"];
     "VI.8.p.1" [style="rounded,filled", fillcolor=orange];
     "VI.1" [URL="/heath/VI/1/", target="_top"];
     "VI.14" [URL="/heath/VI/14/", target="_top"];
     "I.39" [URL="/heath/I/39/", target="_top"];
     "I.14" [URL="/heath/I/14/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "XI.2" [URL="/heath/XI/2/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "XI.7" [URL="/heath/XI/7/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "III.18" [URL="/heath/III/18/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "V.21" [URL="/heath/V/21/", target="_top"];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "III.32" [URL="/heath/III/32/", target="_top"];
     "V.3" [URL="/heath/V/3/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "I.17" [URL="/heath/I/17/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "I.38" [URL="/heath/I/38/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "XIII.13" [style="rounded,filled", fillcolor=lightblue, URL="/heath/XIII/13/", target="_top"];
     "I.12" [URL="/heath/I/12/", target="_top"];
     "XI.1" [style="rounded,filled", fillcolor=orange, URL="/heath/XI/1/", target="_top"];
     "XI.11" [URL="/heath/XI/11/", target="_top"];
     "VI.8" [URL="/heath/VI/8/", target="_top"];
     "I.46" [URL="/heath/I/46/", target="_top"];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "VI.4" [URL="/heath/VI/4/", target="_top"];
     "V.16" [URL="/heath/V/16/", target="_top"];
     "VI.16" [URL="/heath/VI/16/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "I.32" [URL="/heath/I/32/", target="_top"];
     "XI.4" [URL="/heath/XI/4/", target="_top"];
     "I.47" [URL="/heath/I/47/", target="_top"];
     "XI.8" [URL="/heath/XI/8/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "III.1" [URL="/heath/III/1/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "XI.3" [style="rounded,filled", fillcolor=orange, URL="/heath/XI/3/", target="_top"];
     "I.18" [URL="/heath/I/18/", target="_top"];
     "XI.12" [URL="/heath/XI/12/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "V.12" [URL="/heath/V/12/", target="_top"];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "V.2" [style="rounded,filled", fillcolor=orange, URL="/heath/V/2/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "III.31" [URL="/heath/III/31/", target="_top"];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "V.22" [URL="/heath/V/22/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "IV.2" [URL="/heath/IV/2/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "I.28" [URL="/heath/I/28/", target="_top"];
     "VI.17" [URL="/heath/VI/17/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "V.15" [URL="/heath/V/15/", target="_top"];
     "VI.2" [URL="/heath/VI/2/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "III.21" [URL="/heath/III/21/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "I.31" -> "I.23";
     "IV.2" -> "I.23";
     "I.10" -> "I.9";
     "IV.2" -> "III.16.p.1";
     "VI.8" -> "VI.def.1";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "XIII.12" -> "IV.15.p.1";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.2" -> "V.7";
     "VI.14" -> "V.7";
     "V.16" -> "elem.5.17 elem.5.18";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.12" -> "I.post.1";
     "I.16" -> "I.post.1";
     "V.16" -> "V.14";
     "III.18" -> "I.19";
     "III.21" -> "III.20";
     "III.32" -> "III.19";
     "VI.2" -> "V.9";
     "VI.14" -> "V.9";
     "I.32" -> "I.31";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.39" -> "I.31";
     "I.46" -> "I.31";
     "XI.11" -> "I.31";
     "XI.12" -> "I.31";
     "XIII.13" -> "XIII.12";
     "III.31" -> "III.22";
     "III.32" -> "III.22";
     "I.11" -> "I.def.10";
     "I.12" -> "I.def.10";
     "I.13" -> "I.def.10";
     "III.1" -> "I.def.10";
     "III.31" -> "I.def.10";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "V.22" -> "V.4";
     "XI.4" -> "XI.def.3";
     "XI.8" -> "XI.def.3";
     "XI.11" -> "XI.def.3";
     "XIII.13" -> "XI.def.3";
     "XIII.13" -> "VI.8.p.1";
     "VI.2" -> "VI.1";
     "VI.14" -> "VI.1";
     "XIII.13" -> "VI.1";
     "VI.16" -> "VI.14";
     "VI.2" -> "I.39";
     "I.47" -> "I.14";
     "VI.14" -> "I.14";
     "V.16" -> "V.11";
     "VI.1" -> "V.11";
     "VI.2" -> "V.11";
     "VI.14" -> "V.11";
     "XI.8" -> "XI.2";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "I.46" -> "I.34";
     "VI.4" -> "I.34";
     "XI.8" -> "XI.7";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "I.18" -> "I.3";
     "V.8" -> "V.def.4";
     "III.19" -> "III.18";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "V.16" -> "V.21";
     "I.3" -> "I.2";
     "IV.2" -> "III.32";
     "V.4" -> "V.3";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.17" -> "I.13";
     "I.28" -> "I.13";
     "I.29" -> "I.13";
     "I.32" -> "I.13";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.47" -> "I.4";
     "XI.4" -> "I.4";
     "XIII.13" -> "I.4";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.12" -> "I.8";
     "I.23" -> "I.8";
     "III.1" -> "I.8";
     "XI.4" -> "I.8";
     "III.18" -> "I.17";
     "III.31" -> "I.17";
     "VI.4" -> "I.17";
     "I.32" -> "I.29";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.46" -> "I.29";
     "XI.8" -> "I.29";
     "VI.1" -> "I.38";
     "VI.2" -> "I.38";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "I.16" -> "I.15";
     "I.28" -> "I.15";
     "I.29" -> "I.15";
     "XI.4" -> "I.15";
     "I.12" -> "I.10";
     "I.16" -> "I.10";
     "I.13" -> "I.11";
     "I.46" -> "I.11";
     "XI.11" -> "I.11";
     "I.27" -> "I.def.23";
     "XI.11" -> "I.12";
     "XI.2" -> "XI.1";
     "XI.12" -> "XI.11";
     "XIII.13" -> "VI.8";
     "I.47" -> "I.46";
     "I.34" -> "I.26";
     "XI.4" -> "I.26";
     "VI.8" -> "VI.4";
     "XIII.13" -> "VI.4";
     "VI.1" -> "V.16";
     "VI.4" -> "V.16";
     "VI.17" -> "VI.16";
     "I.36" -> "I.33";
     "III.20" -> "I.32";
     "III.22" -> "I.32";
     "III.31" -> "I.32";
     "III.32" -> "I.32";
     "IV.2" -> "I.32";
     "VI.8" -> "I.32";
     "XI.8" -> "XI.4";
     "XI.11" -> "XI.4";
     "XIII.12" -> "I.47";
     "XI.11" -> "XI.8";
     "XI.12" -> "XI.8";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "I.12" -> "I.post.3";
     "XIII.13" -> "III.1";
     "V.4" -> "V.def.5";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "V.22" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "XI.7" -> "XI.3";
     "I.19" -> "I.18";
     "XIII.13" -> "XI.12";
     "V.16" -> "V.20";
     "V.22" -> "V.20";
     "I.28" -> "I.27";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "V.15" -> "V.12";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "I.39" -> "I.cn.1";
     "V.3" -> "V.2";
     "I.8" -> "I.7";
     "I.38" -> "I.36";
     "I.7" -> "I.5";
     "I.19" -> "I.5";
     "III.20" -> "I.5";
     "III.31" -> "I.5";
     "I.39" -> "I.37";
     "I.41" -> "I.37";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.17" -> "I.16";
     "I.18" -> "I.16";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "III.32" -> "III.31";
     "XIII.12" -> "III.31";
     "XIII.13" -> "III.31";
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "VI.4" -> "V.22";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.47" -> "I.cn.2";
     "XIII.13" -> "IV.2";
     "I.29" -> "I.post.5";
     "VI.4" -> "I.post.5";
     "VI.4" -> "I.28";
     "XIII.13" -> "VI.17";
     "I.4" -> "I.cn.4";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "VI.4" -> "VI.2";
     "I.47" -> "I.41";
     "VI.1" -> "I.41";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.17" -> "I.post.2";
     "III.22" -> "III.21";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
   }



Required for
------------

:ref:`XIII.18`