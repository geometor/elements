:order: 5
:number: 594
:type: prop
:tags: line
:dependencies: V.14, VI.17, elem.6.def.3 elem.6.17




.. figure:: XIII.5.graphic.inverted.png

.. _XIII.5:

XIII.5
======

   If a straight line be cut in extreme and mean ratio, and there be added to it a straight line equal to the greater segment, the whole straight line has been cut in extreme and mean ratio, and the original straight line is the greater segment.

For let the straight line AB be cut in extreme and mean ratio at the point C, let AC be the greater segment, and let AD be equal to AC.

I say that the straight line DB has been cut in extreme and mean ratio at A, and the original straight line AB is the greater segment.

For let the square AE be described on AB, and let the figure be drawn.

Since AB has been cut in extreme and mean ratio at C, therefore the rectangle AB, BC is equal to the square on AC. [:ref:`elem.6.def.3 elem.6.17`, VI. 17]

And CE is the rectangle AB, BC, and CH the square on AC; therefore CE is equal to HC.

But HE is equal to CE, and DH is equal to HC; therefore DH is also equal to HE.

Therefore the whole DK is equal to the whole AE.

And DK is the rectangle BD, DA, for AD is equal to DL; and AE is the square on AB; therefore the rectangle BD, DA is equal to the square on AB.

Therefore, as DB is to BA, so is BA to AD. [:ref:`VI.17`]

And DB is greater than BA; therefore BA is also greater than AD. [:ref:`V.14`]

Therefore DB has been cut in extreme and mean ratio at A, and AB is the greater segment. Q. E. D.


Dependency Graph
----------------

.. graphviz::

   digraph {
     rankdir="TB";
     node [shape=box, style=rounded];
     "V.1" [style="rounded,filled", fillcolor=orange, URL="/heath/V/1/", target="_top"];
     "I.cn.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.3/", target="_top"];
     "I.29" [URL="/heath/I/29/", target="_top"];
     "I.7" [URL="/heath/I/7/", target="_top"];
     "I.27" [URL="/heath/I/27/", target="_top"];
     "I.38" [URL="/heath/I/38/", target="_top"];
     "V.16" [URL="/heath/V/16/", target="_top"];
     "V.15" [URL="/heath/V/15/", target="_top"];
     "I.37" [URL="/heath/I/37/", target="_top"];
     "I.34" [URL="/heath/I/34/", target="_top"];
     "V.20" [URL="/heath/V/20/", target="_top"];
     "I.8" [URL="/heath/I/8/", target="_top"];
     "I.def.23" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.23/", target="_top"];
     "elem.6.def.3 elem.6.17" [style="rounded,filled", fillcolor=orange];
     "V.def.4" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.4/", target="_top"];
     "V.21" [URL="/heath/V/21/", target="_top"];
     "I.9" [URL="/heath/I/9/", target="_top"];
     "V.8" [URL="/heath/V/8/", target="_top"];
     "I.post.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.4/", target="_top"];
     "I.2" [URL="/heath/I/2/", target="_top"];
     "V.7" [URL="/heath/V/7/", target="_top"];
     "I.1" [URL="/heath/I/1/", target="_top"];
     "I.def.15" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.15/", target="_top"];
     "V.13" [URL="/heath/V/13/", target="_top"];
     "I.post.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.1/", target="_top"];
     "VI.17" [URL="/heath/VI/17/", target="_top"];
     "VI.14" [URL="/heath/VI/14/", target="_top"];
     "I.cn.4" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.4/", target="_top"];
     "I.26" [URL="/heath/I/26/", target="_top"];
     "I.33" [URL="/heath/I/33/", target="_top"];
     "V.def.7" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.7/", target="_top"];
     "elem.5.20 elem.5.21 elem.5.22 elem.5.23" [style="rounded,filled", fillcolor=orange];
     "I.23" [URL="/heath/I/23/", target="_top"];
     "I.13" [URL="/heath/I/13/", target="_top"];
     "I.cn.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.2/", target="_top"];
     "I.5" [URL="/heath/I/5/", target="_top"];
     "I.11" [URL="/heath/I/11/", target="_top"];
     "V.10" [URL="/heath/V/10/", target="_top"];
     "I.10" [URL="/heath/I/10/", target="_top"];
     "I.4" [URL="/heath/I/4/", target="_top"];
     "I.3" [URL="/heath/I/3/", target="_top"];
     "I.16" [URL="/heath/I/16/", target="_top"];
     "VI.16" [URL="/heath/VI/16/", target="_top"];
     "XIII.5" [style="rounded,filled", fillcolor=lightblue, URL="/heath/XIII/5/", target="_top"];
     "I.31" [URL="/heath/I/31/", target="_top"];
     "I.def.10" [style="rounded,filled", fillcolor=orange, URL="/heath/I/def.10/", target="_top"];
     "V.11" [style="rounded,filled", fillcolor=orange, URL="/heath/V/11/", target="_top"];
     "I.post.2" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.2/", target="_top"];
     "I.post.3" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.3/", target="_top"];
     "VI.1" [URL="/heath/VI/1/", target="_top"];
     "V.9" [URL="/heath/V/9/", target="_top"];
     "I.post.5" [style="rounded,filled", fillcolor=orange, URL="/heath/I/post.5/", target="_top"];
     "elem.5.17 elem.5.18" [style="rounded,filled", fillcolor=orange];
     "I.14" [URL="/heath/I/14/", target="_top"];
     "I.36" [URL="/heath/I/36/", target="_top"];
     "V.12" [URL="/heath/V/12/", target="_top"];
     "I.35" [URL="/heath/I/35/", target="_top"];
     "I.15" [URL="/heath/I/15/", target="_top"];
     "I.cn.1" [style="rounded,filled", fillcolor=orange, URL="/heath/I/cn.1/", target="_top"];
     "I.41" [URL="/heath/I/41/", target="_top"];
     "V.def.5" [style="rounded,filled", fillcolor=orange, URL="/heath/V/def.5/", target="_top"];
     "V.14" [URL="/heath/V/14/", target="_top"];
     "V.8" -> "V.1";
     "V.12" -> "V.1";
     "I.2" -> "I.cn.3";
     "I.14" -> "I.cn.3";
     "I.15" -> "I.cn.3";
     "I.35" -> "I.cn.3";
     "I.33" -> "I.29";
     "I.34" -> "I.29";
     "I.35" -> "I.29";
     "I.8" -> "I.7";
     "I.31" -> "I.27";
     "I.33" -> "I.27";
     "VI.1" -> "I.38";
     "VI.1" -> "V.16";
     "V.16" -> "V.15";
     "VI.1" -> "V.15";
     "I.41" -> "I.37";
     "I.35" -> "I.34";
     "I.36" -> "I.34";
     "I.37" -> "I.34";
     "I.38" -> "I.34";
     "I.41" -> "I.34";
     "V.16" -> "V.20";
     "I.9" -> "I.8";
     "I.11" -> "I.8";
     "I.23" -> "I.8";
     "I.27" -> "I.def.23";
     "XIII.5" -> "elem.6.def.3 elem.6.17";
     "V.8" -> "V.def.4";
     "V.16" -> "V.21";
     "I.10" -> "I.9";
     "V.9" -> "V.8";
     "V.10" -> "V.8";
     "V.14" -> "V.8";
     "V.20" -> "V.8";
     "V.21" -> "V.8";
     "I.14" -> "I.post.4";
     "I.15" -> "I.post.4";
     "I.3" -> "I.2";
     "V.10" -> "V.7";
     "V.15" -> "V.7";
     "VI.14" -> "V.7";
     "I.2" -> "I.1";
     "I.10" -> "I.1";
     "I.11" -> "I.1";
     "I.1" -> "I.def.15";
     "I.3" -> "I.def.15";
     "V.14" -> "V.13";
     "V.20" -> "V.13";
     "V.21" -> "V.13";
     "I.1" -> "I.post.1";
     "I.2" -> "I.post.1";
     "I.5" -> "I.post.1";
     "I.16" -> "I.post.1";
     "XIII.5" -> "VI.17";
     "VI.16" -> "VI.14";
     "I.4" -> "I.cn.4";
     "I.34" -> "I.26";
     "I.36" -> "I.33";
     "V.8" -> "V.def.7";
     "V.13" -> "V.def.7";
     "V.16" -> "elem.5.20 elem.5.21 elem.5.22 elem.5.23";
     "I.31" -> "I.23";
     "I.14" -> "I.13";
     "I.15" -> "I.13";
     "I.29" -> "I.13";
     "I.29" -> "I.cn.2";
     "I.34" -> "I.cn.2";
     "I.35" -> "I.cn.2";
     "I.7" -> "I.5";
     "I.13" -> "I.11";
     "V.14" -> "V.10";
     "V.20" -> "V.10";
     "V.21" -> "V.10";
     "I.16" -> "I.10";
     "I.5" -> "I.4";
     "I.10" -> "I.4";
     "I.16" -> "I.4";
     "I.26" -> "I.4";
     "I.33" -> "I.4";
     "I.34" -> "I.4";
     "I.35" -> "I.4";
     "I.5" -> "I.3";
     "I.9" -> "I.3";
     "I.11" -> "I.3";
     "I.16" -> "I.3";
     "I.26" -> "I.16";
     "I.27" -> "I.16";
     "VI.17" -> "VI.16";
     "I.37" -> "I.31";
     "I.38" -> "I.31";
     "I.11" -> "I.def.10";
     "I.13" -> "I.def.10";
     "V.16" -> "V.11";
     "VI.1" -> "V.11";
     "VI.14" -> "V.11";
     "I.2" -> "I.post.2";
     "I.5" -> "I.post.2";
     "I.16" -> "I.post.2";
     "I.1" -> "I.post.3";
     "I.2" -> "I.post.3";
     "I.3" -> "I.post.3";
     "VI.14" -> "VI.1";
     "VI.14" -> "V.9";
     "I.29" -> "I.post.5";
     "V.16" -> "elem.5.17 elem.5.18";
     "VI.14" -> "I.14";
     "I.38" -> "I.36";
     "V.15" -> "V.12";
     "I.36" -> "I.35";
     "I.37" -> "I.35";
     "I.16" -> "I.15";
     "I.29" -> "I.15";
     "I.1" -> "I.cn.1";
     "I.2" -> "I.cn.1";
     "I.3" -> "I.cn.1";
     "I.14" -> "I.cn.1";
     "I.15" -> "I.cn.1";
     "I.29" -> "I.cn.1";
     "I.35" -> "I.cn.1";
     "I.36" -> "I.cn.1";
     "VI.1" -> "I.41";
     "V.7" -> "V.def.5";
     "V.12" -> "V.def.5";
     "V.13" -> "V.def.5";
     "V.16" -> "V.def.5";
     "VI.1" -> "V.def.5";
     "V.16" -> "V.14";
     "XIII.5" -> "V.14";
   }



Required for
------------

:ref:`XIII.17`, :ref:`XIII.18`