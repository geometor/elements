...
For let the square ``CDEB`` be described on ``CB``; [:ref:`I. 46 <elem.1.46>`] let ``ED`` be drawn through to ``F``, and through ``A`` let ``AF`` be drawn parallel to either ``CD`` or ``BE``. [:ref:`I. 31 <elem.1.31>`]
