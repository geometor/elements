...
For let there be constructed the rectangular parallelogram ``BD`` equal to the rectilineal figure ``A``. [:ref:`I. 45 <elem.1.45>`]
...
But the squares on ``HE``, ``EG`` are equal to the square on ``GH``; [:ref:`I. 47 <elem.1.47>`] ``BE``, ``EF`` together with the square on ``GE`` is equal to the squares on ``HE``, ``EG``.
