...
For let the square ``CEFB`` be described on ``CB``, [:ref:`I. 46 <elem.1.46>`] and let ``BE`` be joined; through ``D`` let ``DG`` be drawn parallel to either ``CE`` or ``BF``, through ``H`` again let ``KM`` be drawn parallel to either ``AB`` or ``EF``, and again through ``A`` let ``AK`` be drawn parallel to either ``CL`` or ``BM``. [:ref:`I. 31 <elem.1.31>`]

Then, since the complement ``CH`` is equal to the complement ``HF``, [:ref:`I. 43 <elem.1.43>`] let ``DM`` be added to each; therefore the whole CM is equal to the whole DF.
