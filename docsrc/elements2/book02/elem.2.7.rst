...
For let the square ``ADEB`` be described on ``AB``, [:ref:`I. 46 <elem.1.46>`] and let the figure be drawn.

Then, since ``AG`` is equal to ``GE``, [:ref:`I. 43 <elem.1.43>`] let ``CF`` be added to each; therefore the whole AF is equal to the whole CE.
