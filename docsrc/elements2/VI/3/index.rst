...
.. container:: center
   ...
   the angle ``ACE`` is equal to the angle ``CAD``. [:ref:`I.29`]
...
.. container:: center
   ...
   the exterior angle ``BAD`` is equal to the interior angle ``AEC``. [:ref:`I.29`]
...
.. container:: center
   ...
   therefore the angle ``ACE`` is also equal to the angle ``AEC``, so that the side ``AE`` is also equal to the side ``AC``. [:ref:`I.6`]
...
But ``AE`` is equal to ``AC``; [:ref:`VI.2`] therefore, as ``BD`` is to ``DC``, so is ``BA`` to ``AC``.
...
For, with the same construction, since, as ``BD`` is to ``DC``, so is ``BA`` to ``AC``, and also, as ``BD`` is to ``DC``, so is ``BA`` to ``AE`` : for ``AD`` has been drawn parallel to ``EC``, one of the sides of the triangle ``BCE``: [:ref:`VI.2`] therefore also, as ``BA`` is to ``AC``, so is ``BA`` to ``AE``. [:ref:`V.11`]

Therefore ``AC`` is equal to ``AE``, [:ref:`V.9`] so that the angle ``AEC`` is also equal to the angle ``ACE``. [:ref:`I.5`]

But the angle ``AEC`` is equal to the exterior angle ``BAD``, [:ref:`I.29`] and the angle ``ACE`` is equal to the alternate angle ``CAD``; [``id``.]
...
.. graphviz::
   ...
   digraph {
     ...
     "VI.3" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/VI/3/", target="_top"];
