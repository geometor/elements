...
For on the straight line ``DF``, and at the points ``D``, ``F`` on it, let there be constructed the angle ``FDG`` equal to either of the angles ``BAC``, ``EDF``, and the angle ``DFG`` equal to the angle ``ACB``; [:ref:`I.23`]


.. container:: center

   therefore the remaining angle at ``B`` is equal to the remaining angle at ``G``. [:ref:`I.32`]


Therefore the triangle ``ABC`` is equiangular with the triangle ``DGF``.

Therefore, proportionally, as ``BA`` is to ``AC``, so is ``GD`` to ``DF``. [:ref:`VI.4`]
...
and the remaining angles will be equal to the remaining angles, namely those which the equal sides subtend. [:ref:`I.4`]
...
And, by hypothesis, the angle ``BAC`` is also equal to the angle ``EDF``; therefore the remaining angle at ``B`` is also equal to the remaining angle at ``E``; [:ref:`I.32`]
...
.. graphviz::
   ...
   digraph {
     ...
     "VI.6" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/VI/6/", target="_top"];
