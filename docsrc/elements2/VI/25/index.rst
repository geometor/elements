...
:tags: line
:dependencies: I.44, I.45, V.16, VI.1, VI.13, VI.18, elem.6.19.p.1
...
Now let ``GH`` be taken a mean proportional to ``BC``, ``CF`` [:ref:`VI.13`], and on ``GH`` let ``KGH`` be described similar and similarly situated to ``ABC``. [:ref:`VI.18`]

Then, since, as ``BC`` is to ``GH``, so is ``GH`` to ``CF``, and, if three straight lines be proportional, as the first is to the third, so is the figure on the first to the similar and similarly situated figure described on the second, [:ref:`elem.6.19.p.1`] therefore, as ``BC`` is to ``CF``, so is the triangle ``ABC`` to the triangle ``KGH``.

But, as ``BC`` is to ``CF``, so also is the parallelogram ``BE`` to the parallelogram ``EF``. [:ref:`VI.1`]

Therefore also, as the triangle ``ABC`` is to the triangle ``KGH``, so is the parallelogram ``BE`` to the parallelogram ``EF``; therefore, alternately, as the triangle ``ABC`` is to the parallelogram ``BE``, so is the triangle ``KGH`` to the parallelogram ``EF``. [:ref:`V.16`]
...
.. graphviz::
   ...
   digraph {
     ...
     "VI.13" [URL="/elements2/VI/13/", target="_top"];
     ...
     "VI.18" [URL="/elements2/VI/18/", target="_top"];
     ...
     "VI.25" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/VI/25/", target="_top"];
     ...
     "I.45" [URL="/elements2/I/45/", target="_top"];
