...
For, since ``AB`` is parallel to ``DC``, and the straight line ``AC`` has fallen upon them, the alternate angles ``BAC``, ``ACD`` are equal to one another. [:ref:`I.29`]
...
.. container:: center
   ...
   therefore the triangle ``ABC`` is equiangular with the triangle ``DCE``; [:ref:`VI.6`] therefore the angle ``ABC`` is equal to the angle ``DCE``.
...
But the angles ``BAC``, ``ABC``, ``ACB`` are equal to two right angles; [:ref:`I.32`]
...
.. container:: center
   ...
   therefore ``BC`` is in a straight line with ``CE``. [:ref:`I.14`]
...
.. graphviz::
   ...
   digraph {
     ...
     "VI.32" [style="rounded,filled", fillcolor=lightblue, URL="/elements2/VI/32/", target="_top"];
